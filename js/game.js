...
async init(){await loadConfig();this.renderMainMenu();this.renderTitleMeta();this._renderUpdateIcon();this._checkForUpdate();
...
const item=(icon,label,sub,onclick,gold)=>'<div onclick="'+onclick+'" style="cursor:pointer;display:flex;align-items:center;gap:12px;background:var(--surface);border:1px solid '+(gold?'var(--gold)':'var(--border)')+';border-radius:var(--radius-sm);padding:13px 16px;margin-bottom:8px;transition:border-color .15s;"><span style="font-size:1.05rem;width:22px;text-align:center;">'+icon+'</span><span style="flex:1;min-width:0;"><span style="font-weight:700;font-size:0.92rem;'+(gold?'color:var(--gold);':'')+'">'+label+'</span>'+(sub?'<br><span style="font-size:0.66rem;color:var(--text2);">'+sub+'</span>':'')+'</span><span style="color:var(--text2);font-size:1.1rem;">›</span></div>';
...
if(auto&&auto.month<=36)h+=item('↩','Continue','Month '+auto.month+' · '+this._archLabel(auto.archetype),'Game.resumeAuto()');
...
calcDebtInterest(){const s=this.state,score=s.personal_credit_score||600,cfo=s._cfo_fulltime?0.8:s._cfo_hired?0.9:1;
...
renderSaves(){const el=document.getElementById('resume-list');if(!el)return;const empty=!this.loadAutoSave()&&!this.loadSaves().length;el.innerHTML='<div style="display:flex;align-items:center;gap:10px;margin:4px 0 14px;"><button class="btn-outline" style="width:auto;margin:0;padding:6px 14px;" onclick="Game.showMainMenu()">← Back</button><span style="font-weight:700;color:var(--accent);font-size:0.95rem;">Continue / Load Game</span></div>'+this._autoSaveHtml()+this._savesHtml()+(empty?'<div style="text-align:center;color:var(--text2);font-size:0.82rem;padding:24px 0;">No saved games yet — start a New Game.</div>':'');},
...
html+='<button class="btn-secondary" onclick="'+(from==='end'?'Game.showScreen(\'end-screen\')':'Game.showMainMenu()')+'">Back</button>';
