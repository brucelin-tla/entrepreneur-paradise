...
this.state.personal_cash=this.state.personal_cash||0;
...
B+=row('Leads','<span style="color:var(--text)">'+(s.leads||0)+'</span>');
B+=row('Customers','<span style="color:var(--text)">'+s.customer_base+'</span>');
B+=row('Staff','<span style="color:var(--text)">'+(s.team_size||0)+'</span>');
}else{
...
const personalExp=(s.living_expenses||0)+(s.lifestyle_expenses||0);
if(sep){const salary=Math.min(s.owner_pay||0,Math.max(0,s.cash));s.cash-=salary;s.personal_cash=(s.personal_cash||0)+salary; // salary is personal income → personal cash
s.personal_cash-=personalExp; // personal pays its own living/lifestyle
if(s.personal_cash<0){const extra=Math.min(-s.personal_cash,Math.max(0,s.cash));s.cash-=extra;s.personal_cash+=extra;}} // if salary fell short, owner draws extra to cover
else{s.cash-=personalExp;}
