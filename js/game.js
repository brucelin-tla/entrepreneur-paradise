...
const topInner=grpTag+selBadge+partialBadge+offBadge+newBadge+repeatBadge;const topRow=topInner?'<div class="card-toprow">'+topInner+'</div>':'';
...
(a.id==='pay_down_debt'?(()=>{const pl=this._debtPaydownPlan(),est=pl.payRev+pl.payLoan;return '<span class="cost-tag cost-cash">'+(est>0?'💵 ~'+this.fmt(est):'already healthy')+'</span>';})():(_cc?'<span class="cost-tag cost-cash">💵 '+this.fmt(_cc)+'</span>':''))+
...
else{btn.textContent='Take Action';if(sec)sec.style.display='block';}},
