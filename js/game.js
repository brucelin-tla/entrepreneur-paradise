...
const tips=[
{k:'toxic_closer',show:!!s._toxic_closer,sel:'#biz-ops',t:'⚠ Your Closer Is a Liability',b:'That high-ticket closer is booking deals on <strong>fake promises and guarantees</strong> — and it\'s catching up with you: churn is climbing, your brand is eroding, and <strong>lawsuit risk keeps rising</strong> every month he stays. He\'s also costing ~$9,000/mo. The fix: open <strong>⚙️ Operations</strong> and run <strong>Restructure &amp; Downsize</strong> (red <strong>⚠ FIRE THE CLOSER</strong> tag) to let him go. Fair warning — he\'ll threaten to sue, so it\'ll cost you legal fees and a spike in lawsuit risk. But the bleeding stops.'},
{k:'overstaffed',show:this._overstaffed(),sel:'#biz-ops',t:'⚠ Payroll Is Bleeding You',b:'Your team is large but the business is <strong>burning cash every month</strong>. There\'s a fast fix: open <strong>⚙️ Operations</strong> and run <strong>Restructure &amp; Downsize</strong> (look for the red <strong>⚠ CUT PAYROLL</strong> tag) — it lets people go and trims your monthly payroll so you stop the bleed. It costs severance and dents morale, but it buys survival. Over-hiring is the mistake; cutting back fast is the recovery.'},
...
return '<span class="cost-tag cost-locked">'+(unhealthy?'⚠ no spare cash':'already healthy')+'</span>';})():(_cc?'<span class="cost-tag cost-cash">💵 '+this.fmt(_cc)+'</span>':''))+
...
if(s._toxic_closer){
// Firing the toxic closer takes priority — and he doesn't go quietly. The fake-promise damage stops and his heavy pay comes off the books, but he lawyers up and threatens to sue.
s._toxic_closer=false;s.team_size=Math.max(0,(s.team_size||0)-1);const closerOpex=Math.min(s.operating_expenses||0,9000);s.operating_expenses=Math.max(0,(s.operating_expenses||0)-closerOpex);
...
s._dyn_narrative='You cut the high-ticket closer loose — and he did NOT go quietly. He lawyered up, threatening a wrongful-termination and unpaid-commission suit: '+this.fmtMoney(legal)+' in legal fees and settlement, and your lawsuit risk jumps. But the fake-promise bleeding stops, payroll drops '+this.fmtMoney(closerOpex)+'/mo, and the team can finally breathe.';}
else{const cut=Math.min(s.team_size||0,4);
if(cut>0){s.team_size=Math.max(0,(s.team_size||0)-cut);const opexCut=Math.min(s.operating_expenses||0,cut*2000);s.operating_expenses=Math.max(0,(s.operating_expenses||0)-opexCut);const severance=cut*1500;s.cash=(s.cash||0)-severance;if(s.cash<0){const unc=this.coverShortfall(-s.cash);s.cash=0;if(unc>0)s.cash=-unc;}
...
s._dyn_narrative='You restructured — let '+cut+' people go and trimmed '+this.fmtMoney(opexCut)+'/mo off payroll. Severance ran '+this.fmtMoney(severance)+'. The team is leaner and the burn is lighter, but morale dipped and more rides on you now. Reinvest the breathing room into systems before you rehire.';}
