...
return'<div class="action-card '+cls+(isNew?' is-new':'')+' fade-in" style="'+(outgrown?'opacity:0.6;':'')+'" onclick="'+onclick+'">'+grpTag+'<h4>'+a.label+selBadge+partialBadge+offBadge+newBadge+repeatBadge+'</h4><p>'+desc+'</p>'+(!done?this.actionPreview(a):'')+'<div class="action-costs">'+
...
'</div>'+(isSel?(()=>{const te=this._turnEnergy();return '<div style="margin-top:7px;padding-top:6px;border-top:1px dashed var(--border);font-size:0.7rem;font-weight:600;color:'+(te.left<0?'var(--red)':te.left<=15?'var(--gold)':'var(--accent)')+';">⚡ This turn: '+te.used+' of '+te.have+' energy committed · '+(te.left<0?Math.abs(te.left)+' over budget!':te.left+' left')+'</div>';})():'')+'</div>';};
...
const avail=actions.filter(a=>!this.isActionCompleted(a)&&!this.isActionLocked(a)).sort((a,b)=>{const pa=isPartialA(a)?1:0,pb=isPartialA(b)?1:0;if(pa!==pb)return pb-pa;const na=isNewA(a)?1:0,nb=isNewA(b)?1:0;if(na!==nb)return nb-na;return urgency(b)-urgency(a);});
...
primaryActionBtn(){const ac=this._activeCats||CATS,unsel=ac.filter(c=>!this.selectedActions[c]);if(!unsel.length)return this.confirmActions();const nxt=this._nextUnselectedCat();if(nxt&&nxt!==this.currentCategory){this.switchCategory(nxt);return this._focusActionList();}return this.confirmActions();},
