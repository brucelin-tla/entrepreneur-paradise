...
{sel:'#tut-result-card',title:'Your results',body:'Each move you made plays out here — what changed and what it cost. <strong>Tap anywhere on a card</strong> to expand it: the exact stat changes (before → after) and a short <strong>lesson</strong> with the real-world tip behind the move. This is where you learn what actually works.'},
{sel:'#tut-result-detail',title:'Tap the card for details',body:'Any card with a <strong>▾ Tap card for details</strong> hint has more underneath — <strong>tap anywhere on the card</strong> to open the full before → after breakdown and the lesson. Tap again to collapse it. Open it whenever you want to understand <em>why</em> something happened.'},
...
const _S0=this.state,_msStart={cash:_S0.cash||0,personalCash:_S0.personal_cash||0,bizUsed:_S0.business_credit_used||0,bizLim:_S0.business_credit_limit||0,avail:_S0.available_credit||0,debt:_S0.total_debt||0,persScore:_S0.personal_credit_score||0};
...
if(af.bizLim!==_cb.bizLim)rows.push(['Business credit limit',this.fmtMoney(_cb.bizLim),this.fmtMoney(af.bizLim)]);
if(af.bizLim>0&&af.bizUtil!==_cb.bizUtil)rows.push(['Business utilization',_cb.bizUtil+'%',af.bizUtil+'%']);
...
let rows='';rows+=posRow('📊','Credit score',st.persScore,s.personal_credit_score||0,true,_n);rows+=posRow('💵','Cash',cashThen,cashNow,true);rows+=posRow('💳','Credit available',bizAvail0+(st.avail||0),bizAvail1+(s.available_credit||0),true);rows+=posRow('🏦','Total debt',st.debt,s.total_debt||0,false);
