...
if(beat&&beat.mentor_line&&!cl2){charEl.style.display='block';charEl.innerHTML='<div class="char-name">Marcus Webb — Mentor</div>'+beat.mentor_line;}
...
return u;};
actions.sort((a,b)=>{const aD=this.isActionCompleted(a),bD=this.isActionCompleted(b),aL=this.isActionLocked(a),bL=this.isActionLocked(b),aA=!aD&&!aL,bA=!bD&&!bL,aO=aA&&this.isActionOutgrown(a),bO=bA&&this.isActionOutgrown(b);
if(aA&&!aO&&!(bA&&!bO))return-1;if(!(aA&&!aO)&&bA&&!bO)return 1;if(aA&&aO&&!bA)return-1;if(!aA&&bA&&bO)return 1;if(aL&&!aD&&bD)return-1;if(aD&&bL&&!bD)return 1;
if(aA&&bA)return urgency(b)-urgency(a);return 0;});
const MAX_SHOW=6,showAll=this._showAllActions;
const visible=showAll?actions:actions.filter((a,i)=>{if(i<MAX_SHOW)return true;if(sel&&sel.id===a.id)return true;return false;});
const hiddenCount=actions.length-visible.length;
let listHtml=visible.map(a=>{const done=this.isActionCompleted(a),locked=this.isActionLocked(a),isSel=sel&&sel.id===a.id;const reason=done?'Already completed':locked?this.getLockedReason(a):'';const outgrown=!done&&!locked&&this.isActionOutgrown(a);const cls=done?'completed-action':isSel?'selected':locked?'locked':'';const onclick=(locked||done)?'':"Game.selectActionPayment('"+this.currentCategory+"','"+a.id+"')";const desc=this.linkTerms(a.description);const rc=(this.state._action_counts||{})[a.id]||0;const repeatBadge=(!a.one_time&&rc>0)?'<span class="repeat-badge">×'+rc+'</span>':'';
...
'</div></div>';}).join('');
if(hiddenCount>0)listHtml+='<div style="text-align:center;padding:10px;"><span style="color:var(--blue);cursor:pointer;font-size:0.85rem;" onclick="Game._showAllActions=true;Game.renderActions();">Show '+hiddenCount+' more actions ▾</span></div>';
else if(showAll&&actions.length>MAX_SHOW)listHtml+='<div style="text-align:center;padding:10px;"><span style="color:var(--text2);cursor:pointer;font-size:0.85rem;" onclick="Game._showAllActions=false;Game.renderActions();">Show less ▴</span></div>';
...
if(action.id==='buy_real_estate'){const mortAmt=success?80000:85000;this.state.real_estate_debt=(this.state.real_estate_debt||0)+mortAmt;this.state.other_monthly_revenue=(this.state.other_monthly_revenue||0)+(success?2500:1200);}
if(action.id==='private_lending')this.state.other_monthly_revenue=(this.state.other_monthly_revenue||0)+(success?2000:800);
...
this.actionHistory.push(action);results.push({action,success,effects,narrative:cat==='lifestyle'?action.narrative:(success?action.narrative_success:action.narrative_failure)});}
...
buildDebrief(){const s=this.state,c=id=>s._completed_actions&&s._completed_actions.includes(id);
const items=[
{done:c('form_llc')||['llc','s_corp','c_corp','multi_entity'].includes(s.entity_structure),name:'Legal Protection',got:'You shielded your personal assets behind an LLC.',miss:'You never formed an LLC — one lawsuit could have taken everything you own.'},
{done:c('build_personal_credit_optimize')||c('build_personal_credit_repair'),name:'Credit Building',got:'You built your credit into an asset that unlocks cheap capital.',miss:'You never invested in your credit — the key that unlocks all good debt.'},
{done:c('payroll_setup'),name:'Pay Yourself Properly',got:'You put yourself on real payroll — clean books and S-Corp ready.',miss:'You never set a real owner salary — leaving tax savings and lender credibility behind.'},
{done:c('s_corp_election'),name:'Tax-Smart Structure',got:'You elected S-Corp and legally cut your self-employment tax.',miss:'You skipped the S-Corp election — a tax lever employees never get.'},
{done:c('setup_accumulation_policy'),name:'The Tax-Free Money Engine',got:'You opened a cash-value policy — your tax-advantaged wealth engine.',miss:'You never started a cash-value policy — the tax-free passive income engine, untouched.'},
{done:(s.insurance_passive_loan_total||0)>0||(s.insurance_cash_value||0)>=100000,name:'Tax-Free Passive Income',got:'Your policy throws off tax-free passive income — money without work.',miss:'You never reached passive tax-free income — the ultimate goal you were building toward.'},
{done:c('buy_real_estate')||(s.real_estate_owned||0)>0,name:'Leverage Into Assets',got:"You used the bank's money to control income-producing property.",miss:'You never used leverage to acquire a cash-flowing asset.'},
{done:c('private_lending'),name:'Become the Bank',got:"You lent capital for returns — money working while you don't.",miss:'You never put your capital to work as a lender.'},
{done:c('asset_protection')||c('multi_entity')||(s.trust_structure&&s.trust_structure!=='none'),name:'Asset Protection',got:"You walled off your wealth so a lawsuit can't reach it.",miss:'Your wealth sat exposed — no trust or protection structure.'},
{done:c('build_offer'),name:'Irresistible Offer',got:'You packaged a premium offer that sells itself.',miss:'You never sharpened your offer — the fastest lever on revenue.'},
{done:c('full_systemization')||(s.key_person_dependency!=null&&s.key_person_dependency<30),name:'Systems = Freedom',got:'You built a business that runs without you — real freedom.',miss:'The business still leans on you — you built a job more than an asset.'}];
const learned=items.filter(i=>i.done),missed=items.filter(i=>!i.done);
let h='<div class="epilogue-box" style="text-align:left;margin-top:16px;"><div style="font-size:1.1rem;font-weight:700;color:var(--gold);margin-bottom:4px;">What You Learned</div><div style="color:var(--text2);font-size:0.85rem;margin-bottom:10px;">'+learned.length+' of '+items.length+' core wealth principles put into practice.</div>';
...
if(missed.length){h+='<div style="color:var(--text2);font-size:0.72rem;text-transform:uppercase;letter-spacing:1px;margin:14px 0 6px;">Left On The Table</div>';for(const i of missed)h+='<div style="margin-bottom:7px;font-size:0.9rem;opacity:0.9;"><span style="color:var(--gold);font-weight:600;">&#9675; '+i.name+'</span> — '+i.miss+'</div>';}
h+='</div>';return h;},
endGame(){
