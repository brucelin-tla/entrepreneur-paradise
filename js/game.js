...
hidePopup(){document.getElementById('popup-container').style.display='none';this._unlockScroll();},
...
TUTORIAL_STEPS:[
{sel:null,title:'Welcome to Entrepreneur Paradise',body:'You have <strong>up to 36 months</strong> to build wealth and a life you love — cash out at the end of <strong>Year 1, 2, or 3</strong> to lock in your score. Let\'s play your first month together so you get a feel for the game.'},
{sel:'#month-label',title:'Where you stand',body:function(){return 'You\'re a brand-new business owner with <strong>'+(37-this.month)+' months</strong> to build something real. Let\'s look at what you\'re starting with.';}},
{sel:'#dash-cash',title:'Your cash',body:function(){return 'You\'ve got <strong>'+this.fmtMoney(this.isSeparated()?(this.state.personal_cash||0):(this.state.cash||0))+'</strong> in the bank. This funds everything until the business pays for itself — don\'t let it hit zero.';}},
...
_tutRecPick(cat){const prefs={marketing:['cold_outreach','basic_social_content','email_campaign'],operations:['do_work_yourself','study_business_content','basic_quality_control'],finance:['establish_business','build_personal_credit','monthly_tax_reserve']}[cat]||[];
...
endTutorial(){this._tutActive=false;this._tutFinalPending=false;const ov=document.getElementById('tut-overlay');if(ov)ov.remove();},
...
const netRow=(label,net,liquid,click,id)=>{const c=net>=0?'var(--accent)':'var(--red)';let v='<span style="color:'+c+'">'+(net>=0?'+':'−')+fmt(Math.abs(net))+'</span>';if(net<0&&liquid>0){const mo=Math.floor(liquid/Math.abs(net));v+='<br><span style="font-size:0.52rem;color:var(--text2);font-weight:400;">~'+mo+'mo left</span>';}return row(label,v,click,id);};
...
{const nwNow=this.calcNetWorth(),nwDelta=(this._nwStart!=null)?Math.round(nwNow-this._nwStart):null,nwCol=nwNow>=0?'var(--accent)':'var(--red)';let nwHtml='<span style="color:'+nwCol+'">'+fmt(nwNow)+'</span>';if(nwDelta!==null&&nwDelta!==0){const dCol=nwDelta>0?'var(--accent)':'var(--red)';nwHtml+=' <span style="font-size:0.55rem;color:'+dCol+';font-weight:600;">'+(nwDelta>0?'▲ +':'▼ −')+fmt(Math.abs(nwDelta))+'</span>';}if(this._reveal('networth'))P+=row('Net Worth',nwHtml,"Game.showAssets()",'dash-networth');}
...
const _dash=document.getElementById(targetId||'stats-dashboard');_dash.style.display='block';_dash.innerHTML=cfoBtn+achBtn+'<div style="display:flex;align-items:stretch;gap:16px;"><div style="flex:1;min-width:0;">'+P+'</div><div style="width:1px;background:var(--border);"></div><div id="biz-col" style="flex:1;min-width:0;">'+B+'</div></div>';},
...
switchCategory(c){this.currentCategory=c;this._showAllActions=false;this.renderStepIndicator();this.renderCategoryTabs();this.renderActions();this.updateConfirmButton();},
...
EPIC_HANDLED:['asset_protection_stack','combined_insurance','debt_restructure','build_personal_credit','pay_down_debt','banking_relationship','fund_accumulation_policy','activate_passive_income','policy_loan'],
...
const specs=[
 // Fix credit first — it's the gate to everything else, and the player can't do these manually while Epic is active.
 {id:'build_personal_credit',need:()=>(s.credit_negatives||0)>0&&!s._credit_repair},
 {id:'debt_restructure',need:()=>util>45&&persRev()>2000},
 {id:'pay_down_debt',need:()=>util>30&&(s.cash||0)>2000},
 {id:'banking_relationship',once:true,need:()=>true},
 {id:'asset_protection_stack',need:()=>this.isSeparated()&&(!stuck||stable)},
...
for(const spec of specs){const a=fin(spec.id);if(!a)continue;
 ...
 const cost=this.actionCashCost(a);
...
// Collapse long descriptions to 2 lines; each card expands on its own so the list stays scannable.
const longDesc=desc.replace(/<[^>]*>/g,'').length>95,descOpen=this._descOpen&&this._descOpen[a.id];
const descHtml=longDesc?'<p class="action-desc'+(descOpen?'':' desc-clamp')+'">'+desc+'</p><span class="desc-toggle" onclick="event.stopPropagation();Game._toggleDesc(\''+a.id+'\')">'+(descOpen?'Show less ▴':'Read more ▾')+'</span>':'<p>'+desc+'</p>';
return'<div class="action-card '+cls+(isNew?' is-new':'')+' fade-in" style="'+(outgrown?'opacity:0.6;':'')+'" onclick="'+onclick+'">'+grpTag+'<h4>'+a.label+selBadge+partialBadge+offBadge+newBadge+repeatBadge+'</h4>'+descHtml+(!done?this.actionPreview(a):'')+'<div class="action-costs">'+
...
((this.actionEnergyCost(a))>0?'<span class="cost-tag cost-energy">⚡-'+this.actionEnergyCost(a)+' energy</span>':'')+
...
// One flat list with group-name tags on each card. Order: your selection → retry (finish what you started) → NEW → other available (by urgency) → locked (unlocks next) → completed.
const avail=actions.filter(a=>!this.isActionCompleted(a)&&!this.isActionLocked(a)).sort((a,b)=>{const sa=sel&&sel.id===a.id?1:0,sb=sel&&sel.id===b.id?1:0;if(sa!==sb)return sb-sa;const pa=isPartialA(a)?1:0,pb=isPartialA(b)?1:0;if(pa!==pb)return pb-pa;const na=isNewA(a)?1:0,nb=isNewA(b)?1:0;if(na!==nb)return nb-na;return urgency(b)-urgency(a);});
...
const s2=this.state,allExec=s2._cro_hired&&s2._coo_hired&&s2._cfo_hired,teamMode=allExec||s2._board_active;let banner='';
...
let rows='';if(s2._cro_hired)rows+=roleRow('📣','CRO','marketing',(this._autoPicked||{}).marketing);if(s2._coo_hired)rows+=roleRow('⚙️','COO','operations',(this._autoPicked||{}).operations);if(s2._cfo_hired)rows+=roleRow('💰','CFO','finance',this._cfoPick);
...
banner='<div style="background:linear-gradient(135deg,rgba(212,175,55,0.12),rgba(59,130,246,0.1));border:1px solid var(--gold);border-radius:var(--radius-sm);padding:12px 14px;margin-bottom:10px;"><div style="font-size:0.84rem;font-weight:700;color:var(--gold);margin-bottom:6px;">'+(s2._board_active?'🏛 Your Board & Team — Plan This Month':'👔 Your Team’s Plan This Month')+'</div>'+rows+'<div style="font-size:0.68rem;color:var(--text2);margin:7px 0 9px;line-height:1.45;">Override any line by picking your own move below — anything you don’t set, your team handles (and it costs you no energy). Your 🏖️ Life action is always your own pick.</div><button onclick="Game.runTeamMonth()" style="width:100%;padding:11px;border:none;border-radius:var(--radius-sm);background:linear-gradient(135deg,var(--gold),#b8932f);color:#1a1205;font-weight:700;font-size:0.9rem;cursor:pointer;">▶ Run the month with this plan</button>'+(allExec?'<div style="text-align:center;margin-top:7px;"><span onclick="Game.toggleFocus()" style="cursor:pointer;font-size:0.72rem;color:var(--blue);font-weight:600;">'+((this._activeCats||CATS).includes('marketing')?'Hide Marketing &amp; Ops menus (let execs handle)':'Show all menus to override Marketing &amp; Ops')+'</span></div>':'')+'</div>';}
let epicPanel='';
if(this.state._epic_life&&this.currentCategory==='finance'){const handled=CONFIG.actions_finance.actions.filter(a=>this.EPIC_HANDLED.includes(a.id)&&!this.isActionCompleted(a)),names=[...new Set(handled.map(a=>a.label))];if(names.length)epicPanel='<div style="background:rgba(245,158,11,0.08);border:1px solid var(--gold);border-radius:var(--radius-sm);padding:10px 12px;margin-bottom:10px;"><div style="font-size:0.8rem;font-weight:700;color:var(--gold);margin-bottom:4px;">🌟 Epic Life Membership handles these for you</div><div style="font-size:0.72rem;color:var(--text2);line-height:1.5;">'+names.join(' · ')+'<br>Your concierge runs the most relevant one each month — you don’t need to pick them.</div></div>';}
...
_toggleDesc(id){this._descOpen=this._descOpen||{};this._descOpen[id]=!this._descOpen[id];this.renderActions();},
...
if(this._cfoHintShown)return curLine;this._cfoHintShown=true;
return curLine+'<p style="margin:1px 0 0;font-size:0.64rem;color:var(--text2);opacity:0.75;">🔒 Hire a fractional CFO to forecast results (this & other actions)</p>';},
...
_nextUnselectedCat(){const ac=this._activeCats||CATS,start=ac.indexOf(this.currentCategory);for(let i=1;i<=ac.length;i++){const c=ac[(start+i)%ac.length];if(c!==this.currentCategory&&!this.selectedActions[c])return c;}return this.selectedActions[this.currentCategory]?null:this.currentCategory;},
...
primaryActionBtn(){const ac=this._activeCats||CATS,unsel=ac.filter(c=>!this.selectedActions[c]);if(!unsel.length)return this.confirmActions();const nxt=this._nextUnselectedCat();if(nxt&&nxt!==this.currentCategory)return this.switchCategory(nxt);return this.confirmActions();},
...
if(nxt&&nxt!==this.currentCategory){btn.textContent='Next: '+CL[nxt]+' →';if(sec)sec.style.display='block';}
else{btn.textContent='End turn now ('+done+'/'+ac.length+') →';if(sec)sec.style.display='none';}},
...
const _energyCost=this.actionEnergyCost(action);if(_energyCost>0&&!_execRun)this.state.energy-=_energyCost;
...
const _en=Math.max(0,Math.min(100,cs.energy||0)),_body=this.lifeDims().Body;const _neglect=Math.max(0,(60-_en))/60*0.6+Math.max(0,(50-_body))/50*0.6;/* 0 (healthy) .. 1.2 (severely run down) */const healthMult=1+_neglect*2.2;/* up to ~3.6x when you're running yourself into the ground */
...
toggleResultDetail(id){const d=document.getElementById(id);if(!d)return;const open=d.style.display!=='none';d.style.display=open?'none':'block';const t=d.previousElementSibling;if(t)t.innerHTML=t.innerHTML.replace(open?'▴':'▾',open?'▾':'▴');},
...
const _baH=r.beforeAfter?'<div style="margin-top:8px;background:rgba(127,127,127,0.08);border-radius:var(--radius-sm);padding:6px 9px;">'+r.beforeAfter.map(row=>'<div style="display:flex;justify-content:space-between;gap:8px;font-size:0.73rem;padding:2px 0;"><span style="color:var(--text2);">'+row[0]+'</span><span style="white-space:nowrap;"><span style="color:var(--text2);">'+row[1]+'</span> <span style="color:'+(row[3]||'var(--accent)')+';font-weight:700;">→ '+row[2]+'</span></span></div>').join('')+'</div>':'';
...
const _detBlock=_det?'<div onclick="Game.toggleResultDetail(\'rdet'+_ci+'\')" style="cursor:pointer;margin-top:7px;font-size:0.72rem;color:var(--blue);font-weight:600;">▾ Details'+(firstLesson?' & lesson':'')+'</div><div id="rdet'+_ci+'" style="display:none;">'+_det+'</div>':'';
...
const capBlock='<div style="display:flex;justify-content:space-between;align-items:flex-end;gap:8px;margin-bottom:7px;padding-bottom:7px;border-bottom:1px solid var(--border);"><span><div style="font-size:0.6rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px;">Accessible capital</div><div style="font-size:1.15rem;font-weight:800;color:var(--accent);">'+fm(accessible)+'</div><div style="font-size:0.58rem;color:var(--text2);">cash + all available credit</div></span><span style="text-align:right;"><div style="font-size:0.6rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px;">Runway</div><div style="font-size:1.05rem;font-weight:800;color:'+runCol+';">'+runway+'</div><div style="font-size:0.58rem;color:var(--text2);">'+(netFlow>=0?'cash-flow positive':'at current burn')+'</div></span></div>';
let spendLine='';if(sp.total>0){const drawn=sp.cash+sp.biz+sp.pers,fee=Math.max(0,drawn-sp.total),src=[];if(sp.cash>0)src.push('cash');if(sp.biz>0)src.push('business credit');if(sp.pers>0)src.push('personal');spendLine='<div style="font-size:0.74rem;color:var(--text2);margin-top:7px;border-top:1px dashed var(--border);padding-top:6px;">Action costs this month: <strong style="color:var(--red);">−'+fm(sp.total)+'</strong>'+(src.length?' · from '+src.join(' + '):'')+(fee>0?' <span style="color:var(--text2);">(+'+fm(fee)+' credit financing fee)</span>':'')+'</div>';}
...
document.getElementById('lifestyle-scores').innerHTML='<div style="display:flex;gap:8px;margin-bottom:8px;grid-column:span 3;"><div class="stat-card" style="flex:1;"><div class="stat-value stat-positive" style="font-size:0.9rem;">'+this.fmtMoney(s.cash)+'</div><div class="stat-label">Cash</div></div><div class="stat-card" style="flex:1;"><div class="stat-value" style="font-size:0.9rem;color:'+(mastery>50?'var(--accent)':mastery>25?'var(--gold)':'var(--red)')+';">'+mastery+'</div><div class="stat-label">Personal Mastery</div></div></div>'+scores.map(st=>'<div class="stat-card"><div class="stat-value stat-'+st.color+'">'+st.value+'</div><div class="stat-label">'+st.label+'</div></div>').join('');
