...
// Gentle Start: thins the action menu for the first 3 months (new founder) — recommended move + a few options, rest behind "Show more". Flip to false to fully disable.
const GENTLE_START=true;
...
const PATCH_NOTES=[
{v:'0.31.0',d:'2026-06-29 20:30',n:[
'Gentler first 3 months: the action menu now leads with a recommended “⭐ Start here” move and a few clear options, with the rest tucked behind “Show more” — no more wall of choices early on.',
'Your suggested opening move is highlighted each month, so there’s always an obvious next step.',
'Nothing is taken away — every option is one tap away under “Show more options,” and the full menu returns from month 4.'
]},
...
this.month=1;this.selectedActions={};this.actionHistory=[];this.eventHistory=[];this.lifestyleHistory=[];this.monthlySnapshots=[];this._playLog=[];this._pendingTax=false;this._lost=false;this.gameOver=false;this._prevAccessible=null;this._taxRescue=0;
...
TUTORIAL_STEPS:[
{sel:null,title:'Welcome to Entrepreneur Paradise',body:'You have <strong>36 months</strong> to build wealth — cash out at the end of Year 1, 2, or 3 to lock in your score. Let\'s play your first month together.'},
{sel:'#month-label',title:'Where you stand',body:function(){return '<strong>'+(37-this.month)+' months</strong> to build something real. Here\'s what you\'re starting with.';}},
{sel:'#dash-cash',title:'Your cash',body:function(){return '<strong>'+this.fmtMoney(this.isSeparated()?(this.state.personal_cash||0):(this.state.cash||0))+'</strong> in the bank — it funds everything until the business pays for itself. Don\'t let it hit zero.';}},
{sel:'#dash-debt',title:'Your debt',body:function(){const d=Math.max(0,(this.state.total_debt||0)-(this.state.business_credit_used||0)-(this.state.business_installment_debt||0)-(this.state.real_estate_debt||0))+(this.state.insurance_loan_balance||0);return 'About <strong>'+this.fmtMoney(d)+'</strong> in credit-card debt. Paying it down and building credit is a big part of the game.';}},
{sel:'#dash-cashflow',title:'Your cash flow',body:function(){const exp=(this.state.living_expenses||0)+(this.state.lifestyle_expenses||0);return 'Living expenses ~<strong>'+this.fmtMoney(exp)+'/mo</strong> — your monthly <strong>burn</strong>. Tap any stat for a plain-English breakdown.';}},
{sel:'#dash-energy',title:'Your energy',body:'Most moves cost <strong>energy</strong>, and it recovers slowly — you can\'t do everything at once.'},
{sel:'#action-list',cat:'marketing',title:'Your first move — Marketing',body:'Each month you pick one move in <span style="color:var(--accent)">Marketing</span>, <span style="color:var(--blue)">Operations</span> and <span style="color:var(--gold)">Finance</span>. Marketing brings in leads. <strong>Tap the highlighted move.</strong>',wait:'select',waitHint:'Tap the highlighted action'},
{cat:'marketing',explain:true,title:'Why this move?',body:'No leads, no business. <strong>Outbound Prospecting</strong> means talking to people directly to fill your pipeline.'},
{sel:'#action-list',cat:'operations',title:'Now — Operations',body:'Operations runs and delivers the work. <strong>Tap the highlighted move.</strong>',wait:'select',waitHint:'Tap the highlighted action'},
{cat:'operations',explain:true,title:'Why this move?',body:'<strong>Study Business Content</strong> is free and builds your <em>systems</em> — learn the craft before you delegate it.'},
{sel:'#action-list',cat:'finance',title:'Now — Finance',body:'Finance is the heart of the game — credit, leverage, and <strong>passive income</strong>. <strong>Tap the highlighted move.</strong>',wait:'select',waitHint:'Tap the highlighted action'},
{cat:'finance',explain:true,title:'Why this move?',body:'<strong>Form an LLC</strong> shields your personal assets and <strong>unlocks the Business side</strong> of your dashboard.'},
{sel:'#confirm-actions-btn',title:'End your turn',body:function(){var b=document.getElementById('confirm-actions-btn');var t=(b&&b.textContent.trim())||'Confirm Actions';return 'All set. <strong>Tap "'+t+'"</strong> to play out the month.';},wait:'endturn',waitHint:'__btn__'},
{sel:'#tut-result-card',_noBack:true,title:'Your results',body:'Your moves play out here. <strong>Tap any card</strong> to see the exact stat changes and a quick <strong>lesson</strong>.'},
{sel:'#month-cash-panel',title:'Your money this month',body:'A monthly summary — accessible capital, runway, and the swing in your credit, cash and debt. Check it to stay solvent.'},
{sel:null,title:'That\'s the loop!',body:'Each month: read, make your three moves, end the turn, learn. Build safeguards <em>before</em> events hit and steer toward passive income. Now go build.'}
],
...
_tutRecPick(cat){const prefs={marketing:['cold_outreach','basic_social_content','email_campaign'],operations:['study_business_content','do_work_yourself','basic_quality_control'],finance:['establish_business','build_personal_credit','monthly_tax_reserve']}[cat]||[];
...
endTutorial(){this._tutActive=false;this._tutFinalPending=false;if(this.state)this.state._tutorial_seen=true;this.autoSave();/* mark done + persist, so it won't replay on refresh */const ov=document.getElementById('tut-overlay');if(ov)ov.remove();},
...
{k:'life',show:(this._activeCats||[]).includes('lifestyle'),sel:'#life-btn',t:'🏖️ Life Check-In Unlocked',b:'Tap this 🏖️ icon for a Life action. Investing across your health, mind, spirit, relationships and experiences builds <strong>Personal Mastery</strong> — which keeps your energy recovering so you can keep acting. And when energy runs low, a Life action restores it (you can even finance one if cash is tight). Don\'t neglect it.'},
...
switchCategory(c){this.currentCategory=c;this._showAllActions=false;this._earlyExpand=false;this.renderStepIndicator();this.renderCategoryTabs();this.renderActions();this.updateConfirmButton();const _b=document.getElementById('confirm-actions-btn');if(_b)_b.classList.remove('btn-flash');},
...
const recBadge=(!done&&!locked&&this._earlyRec&&this._earlyRec===a.id)?'<span class="new-badge" style="background:var(--accent);color:#04130d;">⭐ START HERE</span>':'';
const topInner=recBadge+grpTag+rescueBadge+selBadge+partialBadge+offBadge+newBadge+repeatBadge;const topRow=topInner?'<div class="card-toprow">'+topInner+'</div>':'';
...
// Gentle Start (first 3 months, new founder): float the recommended move to the top with a "⭐ Start here" badge, show only the top few options, tuck the rest behind "Show more". The tutorial month is excluded so nothing it spotlights can vanish. Kill-switch: GENTLE_START.
const earlyThin=GENTLE_START&&this.archetype&&this.archetype.id==='new'&&this.month<=3&&!this._tutActive;
const recId=earlyThin?this._tutRecPick(this.currentCategory):null;this._earlyRec=recId;
if(earlyThin&&recId){const ri=avail.findIndex(a=>a.id===recId);if(ri>0){const[r]=avail.splice(ri,1);avail.unshift(r);}}
// Keep disguised traps out of the curated short list for brand-new players — they stay reachable under "Show more", just not featured. (Stable sort: traps sink to the back, everything else holds its order.)
if(earlyThin)avail.sort((a,b)=>(TRAPS.indexOf(a.id)>=0?1:0)-(TRAPS.indexOf(b.id)>=0?1:0));
// In early-thin mode, cap BOTH the available cards AND the long "Unlocks next" list (finance has ~8 locked rows in month 2 — that grey wall, not the 4 live cards, is what overwhelms). One expander at the bottom reveals everything.
let shownAvail=avail,shownLocked=locked,earlyHidden=0;const EARLY_CAP=4,EARLY_LOCK=2;
if(earlyThin&&!this._earlyExpand){
  // Curated short list = safe (non-trap) options only, capped. Traps + overflow + most locked rows go into the "Show more" bucket. A trap the player already selected stays visible.
  let keep=avail.filter(a=>TRAPS.indexOf(a.id)<0).slice(0,EARLY_CAP);
  if(sel){const sa=avail.find(a=>a.id===sel.id);if(sa&&keep.indexOf(sa)<0)keep.push(sa);}
  shownAvail=keep;
  shownLocked=locked.slice(0,EARLY_LOCK);
  earlyHidden=(avail.length-shownAvail.length)+(locked.length-shownLocked.length);
}
listHtml+=shownAvail.length?shownAvail.map(card).join(''):'<div style="text-align:center;padding:14px;color:var(--text2);font-size:0.8rem;">No moves available here right now.</div>';
if(shownLocked.length)listHtml+='<div style="padding:10px 12px 2px;font-size:0.66rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px;opacity:0.7;">Unlocks next</div>'+shownLocked.map(lockRow).join('');
if(earlyThin&&earlyHidden>0)listHtml+='<div style="text-align:center;padding:10px;"><span style="color:var(--blue);cursor:pointer;font-size:0.85rem;font-weight:600;" onclick="Game._earlyExpand=true;Game.renderActions();">Show '+earlyHidden+' more '+(earlyHidden>1?'options':'option')+' ▾</span></div>';
else if(earlyThin&&this._earlyExpand&&(avail.length>EARLY_CAP||locked.length>EARLY_LOCK))listHtml+='<div style="text-align:center;padding:10px;"><span style="color:var(--text2);cursor:pointer;font-size:0.85rem;" onclick="Game._earlyExpand=false;Game.renderActions();">Show fewer ▴</span></div>';
...
if((s.cash||0)<0){let need=-s.cash;s.cash=0;if(need>0)need=this.coverShortfall(need);if(need>0)need=this._tapTaxReserveToSurvive(need);if(need>0)s.cash=-need;}
...
if(action.id==='buy_real_estate'){const s=this.state;const mult=Math.max(1,Math.min(4,1+(s.monthly_revenue||0)/40000)),lvlRev=Math.min(2.5,this.calcBusinessLevel()),lvlOpx=Math.min(2,this.calcBusinessLevel());const baseEq=success?20000:10000,baseMort=success?80000:85000,baseRev=success?2500:1200,baseOpx=success?1800:2200;s.real_estate_debt=(s.real_estate_debt||0)+Math.round(baseMort*mult);s.real_estate_equity=(s.real_estate_equity||0)+Math.round(baseEq*mult);s.total_debt+=Math.round(baseMort*mult);s.operating_expenses=(s.operating_expenses||0)+Math.round(baseOpx*lvlOpx)+Math.round(baseOpx*(mult-1));s.monthly_revenue=(s.monthly_revenue||0)+Math.round(baseRev*lvlRev);s.other_monthly_revenue=(s.other_monthly_revenue||0)+Math.round(baseRev*mult);
// Depreciation write-off: a non-cash paper deduction (~building value / 27.5yr) that shelters your taxable income — the signature real-estate tax break.
const reDepr=Math.round(baseMort*mult*0.0036);s.operating_expenses=Math.max(0,(s.operating_expenses||0)-reDepr);s._re_depreciation=(s._re_depreciation||0)+reDepr;
if(success)s._dyn_narrative='You closed on a '+this.fmtMoney(Math.round(baseEq*mult)+Math.round(baseMort*mult))+' property — '+this.fmtMoney(Math.round(baseEq*mult))+' equity, '+this.fmtMoney(Math.round(baseMort*mult))+' financed. It cash-flows and appreciates while you sleep — and its <strong>'+this.fmtMoney(reDepr)+'/mo depreciation write-off</strong> shelters your income, cutting your tax bill even though no cash leaves your pocket.';}
...
if(s.cash<0){const _short=Math.abs(s.cash);s.cash=0;let _unc=this.coverShortfall(_short);if(_unc>0)_unc=this._tapTaxReserveToSurvive(_unc);if(_unc>0){s.cash=-_unc;this._pendingLose='Your business ran out of cash and credit to cover this month\'s obligations.';}}
...
const accCol=accessible<=0?'var(--red)':'var(--accent)';/* out of cash + credit → highlight the number red (the run is over) */const capBlock='<div style="display:flex;justify-content:space-between;align-items:flex-end;gap:8px;margin-bottom:7px;padding-bottom:7px;border-bottom:1px solid var(--border);"><span><div style="font-size:0.6rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px;">💰 Accessible capital</div><div id="result-accessible" style="font-size:1.15rem;font-weight:800;color:'+accCol+';white-space:nowrap;">'+fm(accessible)+(accSwing?' <span style="font-size:0.7rem;">'+accSwing+'</span>':'')+'</div><div style="font-size:0.58rem;color:var(--text2);">cash + credit</div></span><span style="text-align:right;"><div style="font-size:0.6rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px;">Runway</div><div id="result-runway" style="font-size:1.05rem;font-weight:800;color:'+runCol+';">'+runway+'</div><div style="font-size:0.58rem;color:var(--text2);">'+(netFlow>=0?'cash-flow positive':'at current burn')+'</div></span></div>';
...
_autoSaveHtml(){const s=this.loadAutoSave();if(!s||s.month>36)return '';return '<div style="max-width:420px;margin:0 auto 14px;text-align:left;"><div style="background:rgba(16,185,129,0.1);border:1px solid var(--accent);border-radius:var(--radius-sm);padding:12px 14px;"><div style="font-size:0.86rem;font-weight:700;color:var(--accent);margin-bottom:2px;">↩ Continue your game</div><div style="font-size:0.72rem;color:var(--text2);margin-bottom:9px;">'+this._archLabel(s.archetype)+' · Month '+s.month+' of 36 · autosaved '+(s.savedAt||'')+'</div><div style="display:flex;gap:8px;"><button class="btn-primary" style="margin:0;flex:1;" onclick="Game.resumeAuto()">Continue →</button><button class="btn-secondary" style="margin:0;width:auto;padding:10px 12px;" onclick="if(confirm(\'Discard your in-progress game and start fresh?\'))Game.discardAuto();">Discard</button></div></div></div>';},
renderSaves(){const el=document.getElementById('resume-list');if(!el)return;el.innerHTML=this._autoSaveHtml()+this._savesHtml();},
