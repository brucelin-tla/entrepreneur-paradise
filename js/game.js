...
if(action.id==='buy_real_estate'){const mortAmt=success?80000:85000;this.state.real_estate_debt=(this.state.real_estate_debt||0)+mortAmt;}
...
// Lead conversion
const baseConv=0.03,skillConv=(s.skill_marketing||0)/300,brandConv=(s.brand_equity||0)/1000,offerB=s._completed_actions&&s._completed_actions.includes('build_offer')?0.05:0,crmB=s._completed_actions&&s._completed_actions.includes('crm_pipeline')?0.05:0;
...
checkEvents(){for(const evt of CONFIG.events.events.filter(e=>this.meetsReq(e.requires))){let p=evt.base_probability;if(evt.probability_scales_with&&evt.scale_factor)p+=(this.state[evt.probability_scales_with]||0)*evt.scale_factor;if(evt.mitigated_by)for(const id of evt.mitigated_by)if(this.state._completed_actions.includes(id))p*=0.5;if(Math.random()<p)return evt;}return null;},
...
for(const r of results){html+='<div class="result-narrative fade-in"><strong>'+r.action.label+'</strong> — '+(r.success?'<span style="color:var(--accent)">Success</span>':'<span style="color:var(--gold)">Partial</span>')+'<br><br>'+r.narrative+'</div><div class="effect-list">';for(const[k,v]of Object.entries(r.effects)){if(typeof v!=='number'||v===0)continue;const abs=Math.abs(v),isMoney=MK.includes(k);if(isMoney&&abs<500)continue;if(!isMoney&&abs<3)continue;const inv=IK.includes(k),color=inv?(v>0?'var(--red)':'var(--accent)'):(v>0?'var(--accent)':'var(--red)');html+='<div class="effect-item"><span>'+this.formatStatName(k)+'</span><span style="color:'+color+';font-weight:600">'+(isMoney?this.fmtMoney(v):(v>0?'+':'')+v)+'</span></div>';}html+='</div>';}
