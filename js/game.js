...
async init(){await loadConfig();this.renderArchetypes();this.renderTitleMeta();this.renderSaves();this._checkForUpdate();
...
this.month=1;this.selectedActions={};this.actionHistory=[];this.eventHistory=[];this.lifestyleHistory=[];this.monthlySnapshots=[];this._playLog=[];this._pendingTax=false;this._lost=false;this.gameOver=false;this._prevAccessible=null;this._taxRescue=0;this._ownerRescue=0;this._lastRunwayMo=null;
...
_checkForUpdate(){try{fetch('version.json?t='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null).then(j=>{if(j&&j.v&&this._verLt(this._curVersion(),j.v))this._showUpdateBanner(j.v);}).catch(()=>{});}catch(e){}},
...
_ngpFmt(id,val){val=+val;if(id==='ngp-score'||id==='ngp-energy')return Math.round(val);return this.fmtMoney(Math.round(val));},
...
h+=sl('💵 Starting cash','ngp-cash',0,150000,10000,1000)+sl('📊 Personal credit score','ngp-score',500,820,670,5)+sl('💳 Available credit','ngp-credit',0,60000,6000,1000)+sl('🏦 Starting debt','ngp-debt',0,60000,4000,1000,'a handicap — start buried if you dare')+sl('🏠 Living expenses / mo','ngp-living',1500,12000,3200,100)+sl('⚡ Starting energy','ngp-energy',30,100,80,5);
...
this.showPopup('🔁 New Game+ — Customize Your Start',h);},
...
is.cash=num('ngp-cash');is.personal_credit_score=num('ngp-score');is.available_credit=num('ngp-credit');is.total_debt=num('ngp-debt');is.debt_breakdown=num('ngp-debt')>0?{credit_card:num('ngp-debt')}:{};is.personal_guarantee_exposure=num('ngp-debt');is.living_expenses=num('ngp-living');is.energy=num('ngp-energy');
...
_epicRoadmapData(){const s=this.state,c=id=>(s._completed_actions||[]).includes(id),sep=this.isSeparated(),ent=s.entity_structure,trust=s.trust_structure,util=this.calcPersUtil();
...
const fundingReady=[
 ...
 N(c('wyoming_holding_llc')||s._holding_company||['c_corp','multi_entity'].includes(ent)||c('setup_family_office'),'Holding company'),
 N(c('banking_relationship')||['trusted','champion'].includes(s._banker_state),'Banking relationship')];
const protection=[
 N(c('combined_insurance'),'Insurance'),
 N(sep,'Legal entity'),
 N(c('hire_general_counsel'),'Contracts & counsel'),
 N((trust&&!['none','basic_llc'].includes(trust))||c('asset_protection_stack'),'Asset-protection trust')];
const expense=[
 N(['s_corp','c_corp','multi_entity'].includes(ent)||c('advanced_tax_strategy'),'Lower taxes'),
 N(c('debt_restructure')||util<=30,'Lower debt service'),
 N(c('monthly_tax_reserve')||(s.tax_reserve||0)>0,'Reserve discipline')];
const reserve=[
 N((s.insurance_cash_value||0)>0||c('fund_accumulation_policy'),'Accumulation policy'),
 N((s.investment_positions||0)>0,'Investments'),
 N((s.personal_cash||0)>=(s.living_expenses||0)*3,'Emergency fund')];
const policyPassive=s._passive_income_active?Math.round((s.insurance_cash_value||0)*0.06/12):0;
...
const coverInc=passiveInc+Math.max(0,(s.monthly_revenue||0)-(s.cogs||0)-(s.operating_expenses||0));
const coverRatio=debtSvc>0?coverInc/debtSvc:null;
const frPct=pctOf(fundingReady),pPct=pctOf(protection),ePct=pctOf(expense),rPct=pctOf(reserve);
const firstUndone=g=>g.find(n=>!n.done);
let stage,nextNode;
if(frPct<100){stage='Funding Ready';nextNode=(firstUndone(fundingReady)||{}).label;}
else if(pPct<100){stage='Protection layer';nextNode=(firstUndone(protection)||{}).label;}
else if(ePct<100){stage='Expense layer';nextNode=(firstUndone(expense)||{}).label;}
else if(rPct<100){stage='Reserve layer';nextNode=(firstUndone(reserve)||{}).label;}
...
html+=section('🛡 Protection layer',D.pPct,'var(--accent)',chips(D.protection));
if(D.pPct>=100)html+=section('🧾 Expense layer',D.ePct,'var(--accent)',chips(D.expense));
else html+='<div style="font-size:0.7rem;color:var(--text2);margin-top:6px;">Protect what you have first — the next layers unlock as each fills.</div>';
if(D.pPct>=100&&D.ePct>=100)html+=section('💎 Reserve layer',D.rPct,'var(--accent)',chips(D.reserve));
if(D.rPct>0||s._passive_income_active){
 const covCol=D.coverRatio==null?'var(--accent)':(D.coverRatio>=1.25?'var(--accent)':D.coverRatio>=1?'var(--gold)':'var(--red)');
 const covTxt=D.coverRatio==null?'no debt':(D.coverRatio>=99?'∞':D.coverRatio.toFixed(2)+'×');
 html+='<div style="margin-top:14px;border-top:1px dashed var(--border);padding-top:8px;">';
 html+='<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;font-size:0.78rem;"><span style="color:var(--text2);">🩺 Debt coverage <span style="font-size:0.64rem;">income ÷ debt service</span></span><span style="font-weight:700;color:'+covCol+';">'+covTxt+'</span></div>';
...
if((s._asset_units||0)>0){const units=s._asset_units||0,kunits=s._keyman_units||0,cov=units>0?Math.round(kunits/units*100):0,covCol=cov>=100?'var(--accent)':cov>=50?'var(--gold)':'var(--red)',reDebt=s.real_estate_debt||0,claims=s._keyman_claims_total||0;
 const rEng=(l,v,c2)=>'<div style="display:flex;justify-content:space-between;font-size:0.74rem;padding:2px 0;"><span style="color:var(--text2);">'+l+'</span><span style="font-weight:700;'+(c2?'color:'+c2+';':'')+'">'+v+'</span></div>';
 html+='<div style="margin-top:14px;border-top:1px dashed var(--border);padding-top:8px;"><div style="font-size:0.82rem;font-weight:700;color:var(--gold);margin-bottom:3px;">🏗️ Leverage Engine</div>';
 html+=rEng('Income properties',units)+rEng('Asset debt',fmt(reDebt))+rEng('🔑 Key-operator coverage',kunits+'/'+units+' · '+cov+'%',covCol);
 if(cov<100)html+='<div style="font-size:0.7rem;color:var(--red);margin-top:3px;line-height:1.4;">⚠ Uninsured operators — if one is lost, the loan stays but its income stops. Cover them: <strong>Finance → Insure Your Key Operators</strong>.</div>';
 if(claims>0)html+='<div style="font-size:0.7rem;color:var(--accent);margin-top:3px;line-height:1.4;">Policies have retired '+fmt(claims)+' of asset debt on operator losses — covering each specific loan, never a windfall.</div>';
 html+='</div>';}
...
const specs=[
 ...
 // Holding company FIRST — then the banking relationship is built on the new LLC (so the bank sees the clean parent entity).
 {id:'wyoming_holding_llc',need:()=>this.isSeparated()&&(!stuck||stable)},
 {id:'banking_relationship',once:true,need:()=>!!s._holding_company||(s._completed_actions||[]).includes('wyoming_holding_llc')},
...
// Key-Man Leverage portfolio: each property is a leveraged, operator-run income asset. Track the unit + its income so the operator-loss event and key-man coverage scale with the portfolio.
s._asset_units=(s._asset_units||0)+1;s._asset_income=(s._asset_income||0)+Math.round(baseRev*mult);
...
{const afford=Math.max(0,(s.monthly_revenue||0)-(s.cogs||0)-(s.operating_expenses||0));const baseTarget=Math.max((s.living_expenses||0)+1500,Math.round((s.monthly_revenue||0)*0.25));
...
const target=Math.min(afford,baseTarget+cushion);s.owner_pay=Math.max(0,Math.round(target));}
...
_snapshot(){return {v:1,gv:(PATCH_NOTES[0]||{}).v,archetype:this.archetype.id,month:this.month,state:this.state,actionHistory:this.actionHistory,eventHistory:this.eventHistory,lifestyleHistory:this.lifestyleHistory,playLog:this._playLog||[]};},
...
_saveDefaults(){return {_asset_units:0,_asset_income:0,_keyman_units:0,_keyman_premium:0,_keyman_claims_total:0,_holding_company:false,_epic_savings_total:0,company_culture:45};},
...
renderSaves(){const el=document.getElementById('resume-list');if(!el)return;el.innerHTML=this._ngPlusCardHtml()+this._autoSaveHtml()+this._savesHtml();},
