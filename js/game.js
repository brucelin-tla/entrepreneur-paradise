...
{sel:'#tut-result-card',title:'Your results',body:'Each move you made plays out here — what changed and what it cost. This is where you learn what actually works.'},
{sel:'#tut-result-detail',title:'Tap for the full breakdown',body:'Tap <strong>▾ Details & lesson</strong> on any result to expand the exact stat changes (before → after) and a short <strong>lesson</strong> with the real-world tip behind the move. Open it whenever you want to understand <em>why</em> something happened.'},
...
P+=subLab('Capacity')+gauge('⚡ Energy',enReal,enC,enSub,'dash-energy');
...
{const oeNow=s.capital_account||0,oeDelta=(this._oeStart!=null)?Math.round(oeNow-this._oeStart):null;let oeHtml=m(oeNow,oeNow>=0?'var(--text)':'var(--red)');if(oeDelta!==null&&oeDelta!==0){const dCol=oeDelta>0?'var(--accent)':'var(--red)';oeHtml+='<br><span style="font-size:0.52rem;color:'+dCol+';font-weight:600;">'+(oeDelta>0?'▲ +':'▼ −')+fmt(Math.abs(oeDelta))+'</span>';}B+=row('Owner Equity',oeHtml,"Game.showRevenue()");}
...
confirmActions(){const ac=this._activeCats||CATS,count=Object.keys(this.selectedActions).length,total=ac.length;if(count<total){const missing=ac.filter(c=>!this.selectedActions[c]).map(c=>CL[c]).join(', ');if(!window.confirm("You haven't chosen an action for: "+missing+" ("+count+"/"+total+" selected). End the month anyway?"))return;}
const te=this._turnEnergy();if(te.left<0){if(!window.confirm('⚠ Burnout warning\n\nYour moves this month need '+te.used+' energy but you only have '+te.have+'. You\'ll push into the red ('+te.left+') — and running on empty makes you much more likely to get sick this month.\n\nPush through anyway?'))return;}
this.resolveMonth();},
...
const _detBlock=_det?'<div'+(_firstCard?' id="tut-result-detail"':'')+' onclick="Game.toggleResultDetail(\'rdet'+_ci+'\')" style="cursor:pointer;margin-top:7px;font-size:0.72rem;color:var(--blue);font-weight:600;">▾ Details'+(firstLesson?' & lesson':'')+'</div><div id="rdet'+_ci+'" style="display:none;">'+_det+'</div>':'';
