...
openDealControl(cat,id){const s=this.state,fm=v=>this.fmtMoney(Math.round(v));this._dealCat=cat;
 ...
 let canGo=true;
 if(ptype==='property'){const r=this.dealRoll(id),down=Math.max(10,Math.min(50,t.downPct||20))/100;
  ...
  h+='</div>';
  h+=slider('Down payment — your cash in the deal',10,50,Math.round(down*100),Math.round(down*100)+'% = '+fm(cashDown)+' down','downPct');/* slider LAST, right above Confirm — all the moving parts in thumb reach */}
...
const r=this.dealRoll('buy_real_estate'),t=this._dealTerm('buy_real_estate'),down=Math.max(0.1,Math.min(0.5,(t.downPct||20)/100));
...
const r=this.dealRoll('buy_str'),t=this._dealTerm('buy_str'),down=Math.max(0.1,Math.min(0.5,(t.downPct||20)/100));
...
if(s._credit_tight&&(s.real_estate_debt||0)>0&&(s._asset_units||0)>0&&!s._re_squeeze_cycle){s._re_squeeze_cycle=true;
 ...
 if(ltv>0.90&&reDSCR<1.1&&reserves<reSvc*6){
  ...
  if(s._keyman_active&&(s._keyman_units||0)>0){s._keyman_units=Math.max(0,(s._keyman_units||0)-1);this._keymanReconcile();}/* sold unit no longer needs coverage — stop paying its premium */
