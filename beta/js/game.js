...
const CREDIT_APPROVAL=['bank_personal_loan','business_credit_line','debt_restructure'];
const LOAN_APPROVAL=['bank_personal_loan'];/* term loans underwritten on DTI; the rest of CREDIT_APPROVAL is revolving credit underwritten on utilization. debt_restructure qualifies you for BOTH a line and a loan, so it's underwritten on the worse of the two (see resolveMonth). */
...
{sel:'#dash-cashflow',title:'Your burn — the clock',body:function(){const exp=(this.state.living_expenses||0)+(this.state.lifestyle_expenses||0);return 'You spend ~<strong>'+this.fmtMoney(exp)+'/mo</strong> just to live. Your cash drops by this every month until revenue covers it — that\'s the clock you\'re racing.';}},
...
{sel:'#biz-ops',t:'Your funnel: Leads → Customers → Staff',b:'This is what drives the money. <strong>Leads</strong> come from Marketing and convert into paying <strong>Customers</strong>; your <strong>Staff</strong> delivers the work. Remember Marketing\'s three jobs: <strong style="color:var(--accent);">Get Leads</strong> to fill the pipeline, <strong style="color:var(--blue);">Build Trust</strong> so more convert (and pay more), and <strong style="color:var(--gold);">Close the Sale</strong>. Rotate between them — leads go cold if you don\'t convert them.'}
]},
...
// Debt restructure qualifies you for BOTH a business line (utilization) and a working-capital loan (DTI), so it's gated on the WORSE of the two — but a lending expert structures the file for you, so it gets a modest uplift over a cold DIY application. Still fails when both util and DTI read badly (exactly when a line/loan would also be declined).
_restructureApprovalChance(){return Math.min(0.92,Math.min(this._creditApprovalChance(false),this._creditApprovalChance(true))*1.35);},
...
const success=cat==='lifestyle'?true:(_isCreditApp?(Math.random()<_apprCh):(Math.random()<((action.success_rate||0.7)*penalty+skillBonus-diminishing+retryBoost+earlyBoost)));// life actions always land (consistent with the quarterly check-in) — you don't randomly "fail" a workout or a family trip
