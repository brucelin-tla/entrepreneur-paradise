...
P+=row('Capital',m(persCash+persAvail,cashCol(persCash+persAvail)),null,'dash-cash','p_cash');
...
// Capacity block: Freedom, then Personal Mastery (both always shown now, no toggle needed), Energy last/lowest.
P+=subLab('Capacity');
...
P+=gauge('⚡ Energy',enReal,enC,enSub,'dash-energy',enReal<=45);
...
B+=row('Capital',m(bizCash+bizAvail,cashCol(bizCash+bizAvail)));
...
// Marketing & Operations readout: Customers (headline) — Brand Equity + Leads behind the group toggle.
...
// Systems Maturity (headline) — Staff + Culture behind the group toggle. Founder Freedom lives on the
// Personal side now. Regardless of LLC status, same as Founder Freedom used to be — a founder/ops
// maturity read, not gated on having separated business finances yet.
if(this._reveal('mastery')){
B+=gauge('⚙️ Systems Maturity',sysMat,sysC);
if(this._dashOpen.business&&(s.team_size||0)>0){
