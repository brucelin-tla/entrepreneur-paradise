...
openPolicyControl(){const s=this.state,fm=v=>this.fmtMoney(v);
 ...
 if(!s._polStaged)s._polStaged={loanType:(curVar?'variable':'wash'),loanAmt:0,repayAmt:0,fundRate:Math.round((s._policy_fund_rate||0.15)*100),passiveOn:curActive};
 ...
 h+=row('Cash value',fm(cv),'var(--gold)')+row('Net cash value (value − loan)',fm(cv-netLoan),'var(--accent)')+row('Borrowable now',fm(borrow),'var(--accent)')+(charge>0?row('Surrender charge','−'+fm(charge),'var(--red)'):'');
...
// Fold-in offer used to be a ONE-SHOT check fired only the instant fund_accumulation_policy succeeded
// (and only if tax_reserve was already >0 at that exact moment) — so a reserve that built up before the
// policy existed, or that started accruing only after, never got offered at all (owner-reported bug,
// 2026-07-04: "somehow still have tax reserve even though I have a cash value policy"). Now it re-checks
// every month, with a 3-month cooldown so it doesn't nag if declined, and fires for ANY order of
// policy-then-reserve or reserve-then-policy as long as both exist and there's something worth folding.
if((s.tax_reserve||0)>0&&(s._auto_fund_insurance||(s.insurance_cash_value||0)>0)&&!this._pendingReserveFold&&(this.month-(s._reserveFoldAskedMonth||0))>=3){this._pendingReserveFold=true;s._reserveFoldAskedMonth=this.month;}
