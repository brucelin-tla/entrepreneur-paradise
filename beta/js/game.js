...
const PATCH_NOTES=[
{v:'0.68.7',d:'2026-07-03 09:10',n:[
'💼 <strong>My Real Finances — honest assumptions</strong> — the policy illustration now spells out "What has to be true": index crediting is a long-run average (0% floor to cap in real years), policy costs aren\'t in the simple math (early-year value runs below premiums paid), funding discipline for 10+ years is the price of entry, and tax-free holds only while the policy stays in force. Growth horizons under ~10 years get a heads-up.']},
...
rmPolicy(){const d=this._rmData()||{},fm=v=>this._rmFmt(v);const pol=d.policy||{};
 ...
 {const rr=CRD/12,nn=st.years*12,grow=Math.pow(1+rr,nn),projCV=Math.round(effCV*grow+(st.fund>0?st.fund*((grow-1)/rr):0)),projPass=Math.round(projCV*CRD/12);
  ...
  h+='<div style="padding:4px 12px;background:rgba(245,200,66,0.06);border:1px solid var(--gold);border-radius:8px;margin-bottom:8px;"><div class="breakdown-row"><span>Cash value in '+st.years+' yrs</span><span style="color:var(--gold);font-weight:800;">'+fm(projCV)+'</span></div><div class="breakdown-row"><span>Passive it would throw off</span><span style="color:var(--accent);font-weight:800;">'+fm(projPass)+'/mo</span></div><div class="breakdown-detail">Tax-free, compounding at a steady ~6%/yr <em>illustration</em> rate, before policy costs — funding it is what builds the passive engine.'+(st.years<10?' <span style="color:var(--gold);font-weight:700;">⚠️ Under ~10 years, costs and surrender charges typically eat most of the compounding — this is a long-game vehicle.</span>':'')+'</div></div>';}
 h+='<div style="padding:9px 12px;background:var(--surface);border:1px solid var(--border);border-left:3px solid var(--gold);border-radius:8px;margin-bottom:8px;font-size:0.66rem;color:var(--text2);line-height:1.55;"><div style="font-size:0.6rem;text-transform:uppercase;letter-spacing:0.6px;color:var(--gold);font-weight:700;margin-bottom:4px;">📋 What has to be true for this to work</div>'
 +'• <strong>Steady crediting is an illustration, not a promise</strong> — real index years swing between the 0% floor and the cap; ~6%/yr is a long-run average.<br>'
 +'• <strong>Policy costs aren\'t in this simple math</strong> — insurance charges and fees mean real early-year cash value runs <em>below</em> what you paid in, often for 5–10 years.<br>'
 +'• <strong>Funding discipline is the price of entry</strong> — this works if you commit to the premium for 10+ years; quitting early means surrender charges on top of sunk costs.<br>'
 +'• <strong>“Tax-free” holds only while the policy stays in force</strong> — lapse with a loan outstanding and the gains become taxable income. (That\'s why the wash loan is the safe default.)</div>';
...
startSimProfile(kind){
 ...
 const PANEL_ACTS=['policy_loan','activate_passive_income','velocity_banking','buy_real_estate','buy_str','private_banking','cash_out_refi','equipment_financing'];/* panel-routed for humans — a bot queues them directly */
 let simErr=null;
