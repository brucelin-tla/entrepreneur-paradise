...
if(action.id==='fund_accumulation_policy'&&success){const s=this.state;s._auto_fund_insurance=true;s._policy_open_month=this.month;/* start the surrender-charge clock */const cur=Math.round(Math.min((s.monthly_revenue||0)*0.15,this.calcBusinessLevel()*5000));s._dyn_narrative='Policy opened and automatic funding is on — about '+this.fmtMoney(cur)+'/mo (scaling up with your revenue) flows into your policy, a ~2% cost of insurance off the top. Every contribution compounds tax-free and grows your borrowing base.';if((s.tax_reserve||0)>0)this._pendingReserveFold=true;/* offer to roll the existing tax reserve into the new policy */}
