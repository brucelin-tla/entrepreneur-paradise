...
RM_APR:{cards:0.24,mortgage:0.07,personal:0.12,auto:0.08,student:0.065},RM_TERM:{mortgage:360,personal:60,auto:60,student:120},/* today's common APRs + standard amortization terms — demo/education, not exact */
...
rmIntake(){if(!this._rmUnlocked())return this.showCode();const d=this._rmData()||{},pol=d.policy||{},re=d.re||{};
 ...
 const DT=[['cards','Credit cards',50000,500],['mortgage','Mortgage',1000000,5000],['personal','Personal loan',60000,1000],['auto','Auto loan',80000,1000],['student','Student loans',200000,2000]];
...
rmSaveIntake(){const n=id=>{const e=document.getElementById(id);const v=e?parseFloat(e.value):NaN;return isFinite(v)?Math.max(0,v):0;};
 const DT=[['cards','Credit cards'],['mortgage','Mortgage'],['personal','Personal loan'],['auto','Auto loan'],['student','Student loans']];
...
rmPolicy(){const d=this._rmData()||{},fm=v=>this._rmFmt(v);const pol=d.policy||{};
 if(!this._rmPol)this._rmPol={cv:Math.round(+pol.cashValue||0),loan:Math.round(+pol.loan||0),surr:Math.round(+pol.surrender||0),type:'fixed'};const st=this._rmPol;st.cv=Math.max(0,Math.round(st.cv||0));st.surr=Math.max(0,Math.min(Math.round(st.surr||0),st.cv));st.loan=Math.max(0,Math.min(Math.round(st.loan||0),st.cv));if(st.type!=='variable')st.type='fixed';
 const cv=st.cv,loan=st.loan,surr=st.surr,isVar=st.type==='variable',rate=isVar?0.08:0.06,base=Math.max(0,cv-loan),borrowable=Math.max(0,Math.round(cv*0.9)-surr-loan),headroom=Math.max(0,Math.round(cv*0.85)-surr-loan),passiveMo=Math.max(0,Math.min(Math.round(base*rate/12),headroom)),passiveYr=passiveMo*12;
 ...
 h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:4px;">Loan / crediting type</div><div style="display:flex;gap:8px;margin-bottom:6px;">'+tBtn('fixed','Fixed · ~6%','steady, lapse-proof')+tBtn('variable','Variable · ~8%','index-linked — higher, market risk')+'</div>';
 h+='<div style="font-size:0.62rem;color:var(--text2);margin-bottom:10px;line-height:1.35;">'+(isVar?'⚠️ Variable can pay more in good years, but a downturn can stall growth while a loan keeps accruing — real policies can lapse if pushed too hard.':'Fixed keeps it steady and hard to lapse — the classic “wash loan” approach.')+'</div>';
 ...
 h+='<div style="font-size:0.64rem;color:var(--text2);line-height:1.4;margin-top:6px;">Passive draws from your value minus any loan at ~'+Math.round(rate*100)+'%/yr; the surrender charge and loan both reduce what you can draw.</div>';
...
rmPolType(t){if(this._rmPol)this._rmPol.type=(t==='variable'?'variable':'fixed');this.rmPolicy();},
