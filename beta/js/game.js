...
const pb=Math.max(0,Math.floor((s.insurance_cash_value||0)*0.9)-(s.insurance_loan_balance||0)),res=Math.max(0,s.tax_reserve||0),cashAvail=Math.max(0,payCash);
...
const drawWaterfall=(owe)=>{let uP=0,uR=0,uC=0,rem=owe;const pb=Math.max(0,Math.floor((s.insurance_cash_value||0)*0.9)-(s.insurance_loan_balance||0));if(rem>0&&pb>0){uP=Math.min(rem,pb);s.insurance_loan_balance=(s.insurance_loan_balance||0)+uP;rem-=uP;}if(rem>0&&(s.tax_reserve||0)>0){uR=Math.min(rem,s.tax_reserve);s.tax_reserve-=uR;rem-=uR;}if(rem>0){const c=Math.max(0,s[cashKey]||0);uC=Math.min(rem,c);s[cashKey]=(s[cashKey]||0)-uC;rem-=uC;}return{uP,uR,uC,unpaid:rem};};
