...
actionCashCost(a){if(a.id==='debt_restructure')return a._epic?0:this._debtRestructureFee();/* min($2k, 10% success fee on the credit/loan qualified); free for Epic members */if(a.id==='buy_real_estate'||a.id==='buy_str'){const r=this.dealRoll(a.id),t=this._dealTerm(a.id);return Math.round(r.price*Math.max(0.1,Math.min(0.5,(t.downPct||20)/100)));}if(a.id==='equipment_financing'){const _it=this.state._equip_choice&&(a.catalog||[]).find(x=>x.id===this.state._equip_choice);return _it?Math.round(_it.price*0.10):0;}/* equipment down payment runs through the normal funding waterfall (biz credit → biz cash → personal LAST), same as property deals *//* property buys cost the HONEST down payment on this month's deal at the player's chosen % */if(a.category==='lifestyle'&&this.state._epic_life){const _lb=a.cash_cost||0;return _lb?Math.round(_lb*0.8):0;}/* Epic members: 20% off lifestyle upgrade upfront cost */const base=a.cash_cost||0;if(!base)return 0;const retry=this.isRetry(a)?0.5:1;if(a.one_time||(a.category!=='marketing'&&a.category!=='operations'))return Math.round(base*retry);return Math.round(base*Math.min(5,Math.max(1,0.4+this.calcBusinessLevel()*0.6))*retry);},
