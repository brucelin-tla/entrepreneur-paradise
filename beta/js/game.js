...
openPolicyControl(){const s=this.state,fm=v=>this.fmtMoney(v);
 ...
 // Stage everything (loan type + funding % + loan amount + passive on); nothing moves until Confirm. Seed from current on first open.
 if(!s._polStaged)s._polStaged={loanType:(curVar?'variable':'wash'),loanAmt:0,fundRate:Math.round((s._policy_fund_rate||0.15)*100),passiveOn:curActive};
 const st=s._polStaged;st.loanAmt=Math.max(0,Math.min(Math.round(st.loanAmt||0),borrow));const isVar=st.loanType==='variable';
 ...
 const curPassive=curActive?this._policyPassiveMonthly():0,newPassive=(st.passiveOn&&cv>=5000)?this._policyPassiveMonthly(loan+st.loanAmt,isVar):0;/* staged loan TYPE moves the projection too — switch Wash↔Index and watch the passive number change before confirming */
 ...
 h+=row('Cash value',fm(cv),'var(--gold)')+row('Borrowable now',fm(borrow),'var(--accent)')+(charge>0?row('Surrender charge','−'+fm(charge),'var(--red)'):'');
...
polStageLoan(v){const s=this.state;if(s._polStaged)s._polStaged.loanAmt=Math.max(0,Math.round(+v||0));this.openPolicyControl();},
...
setActionFilter(key){this._actionFilter=key;this.renderActions();},
...
const peeked=!!(this._peeked&&this._peeked[this.currentCategory+':'+a.id]);const expanded=isSel||peeked;
...
// Peek toggle: preview the description/impact WITHOUT selecting — lets you compare a couple of cards
// before committing to one. Stops the click from bubbling up to the card's own select handler.
const peekBtn=(!done&&!locked&&!isSel)?'<span onclick="event.stopPropagation();Game.togglePeek(\''+this.currentCategory+'\',\''+a.id+'\')" style="cursor:pointer;font-size:0.6rem;color:var(--blue);font-weight:600;margin-left:6px;white-space:nowrap;">'+(peeked?'▾ hide':'ⓘ preview')+'</span>':'';
...
// Collapsed to just the title + top-row badges + cost tags until picked — a long list scans and scrolls
// faster when only the name is competing for attention. Tapping a card selects it AND expands its
// description/preview right there, so you read the full pitch on the one you're actually considering.
// A "ⓘ preview" link on unselected cards expands the same detail WITHOUT selecting, for comparing options.
return'<div class="action-card '+cls+(isNew?' is-new':'')+' fade-in" style="'+(outgrown?'opacity:0.6;':'')+'" onclick="'+onclick+'">'+topRow+'<h4>'+a.label+peekBtn+'</h4>'+(expanded?('<p>'+desc+'</p>'+(!done&&!locked?this.ownedPreview(a.id):'')+(!done?this.actionPreview(a):'')):'')+'<div class="action-costs">'+
...
const availAll=actions.filter(a=>!this.isActionCompleted(a)&&!this.isActionLocked(a)).sort((a,b)=>{const pa=isPartialA(a)?1:0,pb=isPartialA(b)?1:0;if(pa!==pb)return pb-pa;const na=isNewA(a)?1:0,nb=isNewA(b)?1:0;if(na!==nb)return nb-na;/* a just-unlocked action is pinned on top for its debut month only — after that it joins the shuffle */const ta=_takenBefore(a)?1:0,tb=_takenBefore(b)?1:0;if(ta!==tb)return ta-tb;/* chores you've already run sink below the fresh pool */return _shufKey(a)-_shufKey(b);});
// (No "affordable-first" sort tier: isActionLocked() already excludes anything you can't afford from
// this list entirely — every card here is by definition already affordable, so that tier would be a no-op.)
...
// Simple filter chips above the list: All / never taken / opens more of the tree. (Affordability isn't
// a useful filter here — see the note above, everything in this list already clears that bar.) Only
// thins the fresh-pool view for DISPLAY — _freshLeft/_ownsInvest below still read the unfiltered
// availAll, so the "grow the portfolio" banner reflects the real state, not just the current filter.
this._actionFilter=this._actionFilter||'all';
const neverTaken=a=>((this.state._action_counts||{})[a.id]||0)===0;
let avail=availAll;
if(this._actionFilter==='untried')avail=availAll.filter(neverTaken);
else if(this._actionFilter==='gateway')avail=availAll.filter(a=>this._isGateway(a));
const filterChip=(key,label)=>'<div class="lb-seg '+(this._actionFilter===key?'active':'')+'" onclick="Game.setActionFilter(\''+key+'\')">'+label+'</div>';
const filterBar=availAll.length?'<div class="lb-scope" style="margin-bottom:8px;">'+filterChip('all','All')+filterChip('untried','✨ Untried')+filterChip('gateway','🔑 Opens more')+'</div>':'';
...
const _freshLeft=availAll.some(a=>((_cnts[a.id]||0)===0)&&!isPartialA(a));
const _ownsInvest=availAll.some(a=>INVEST_OWN[a.id]&&(_cnts[a.id]||0)>0);
...
listHtml+=filterBar;
listHtml+=avail.length?avail.map(card).join(''):'<div style="text-align:center;padding:14px;color:var(--text2);font-size:0.8rem;">No '+(this._actionFilter==='untried'?'untried':this._actionFilter==='gateway'?'gateway':'')+' moves right now'+(this._actionFilter!=='all'?' — try “All”.':'.')+'</div>';
...
_dealTerm(id){const d={buy_real_estate:{downPct:20},buy_str:{downPct:20},private_banking:{drawPct:50},cash_out_refi:{ltvPct:70}};return Object.assign({},d[id]||{},(this.state._dealTerms||{})[id]||{});},
...
pal:'<div style="font-size:0.72rem;font-weight:800;color:var(--gold);margin-bottom:5px;">HOW TO SIZE YOUR DRAW</div><div style="font-size:0.72rem;color:var(--text2);line-height:1.55;">The line lets you borrow against a portfolio that <strong style="color:var(--text);">stays invested and keeps compounding</strong> — liquidity without selling, and no capital-gains tax. The interest roughly cancels what the portfolio earns, so the line itself is nearly free carry. The whole game is the draw:<br><br>• <strong style="color:var(--text);">Draw low (≤55%)</strong> — even a deep crash can\'t margin-call you. Sleep-at-night leverage.<br>• <strong style="color:var(--text);">Draw high (&gt;65%)</strong> — a market drop pushes the loan past the pledged value\'s limit → <strong style="color:var(--red);">MARGIN CALL</strong>: the bank force-sells your portfolio at the bottom.<br><br>Rule of the wealthy: the line is dry powder for strong deals — never draw it to the max just because you can.</div>',
refi:'<div style="font-size:0.72rem;font-weight:800;color:var(--gold);margin-bottom:5px;">HOW TO READ YOUR LTV HEADROOM</div><div style="font-size:0.72rem;color:var(--text2);line-height:1.55;">Loan-to-value = your total property debt ÷ what the portfolio is worth. The refi pulls equity out as <strong style="color:var(--text);">tax-free loan cash</strong> (a loan, not a sale — no capital gains) by pushing your LTV up toward the 75% ceiling.<br><br>• <strong style="color:var(--text);">Stop around 60–65%</strong> and you keep a crash-proof cushion — the BRRRR engine with staying power.<br>• <strong style="color:var(--text);">Pull to the 75% max</strong> and you\'ve recycled the most capital — but one downturn puts you underwater and a forced sale on the table.<br><br>Recycle capital, never your cushion. The pull only makes sense if the cash goes into something that cash-flows.</div>'},
...
// RETIREMENT PLAN — the pre-tax sweep. A slice of profit (≈8% of revenue, capped ~$1,900/mo to echo the elective-deferral limit) goes in PRE-TAX: the deduction means only the after-tax portion (contribution×(1−tax_rate)) actually leaves cash, while the FULL contribution lands in _retirement_balance and compounds tax-deferred (~6%/yr). A deductible dollar becomes a compounding asset for ~75¢.
if(s._retirement_active){const contribution=Math.min(Math.round((s.monthly_revenue||0)*0.08),1900);if(contribution>0){const cashCost=Math.round(contribution*(1-(s.tax_rate||0.25)));s.cash=(s.cash||0)-cashCost;s._retirement_balance=(s._retirement_balance||0)+contribution;}if((s._retirement_balance||0)>0)s._retirement_balance=Math.round((s._retirement_balance||0)*1.005);}
...
// Episodic structure: Part 1 (m1–18) ends in a themed FINALE at m18 (the payoff — your wealth machine is built), with a carry-state handoff into Part 2 (m19–36). Month 12 is NO LONGER a checkpoint screen — the year-end tax bill already fired above (that's "April tax season"), so m12 just quietly auto-saves and continues (no off-ramp before the payoff). m24 stays a light Part-2 re-hook checkpoint; m36 goes straight to the final score.
if(this.month===12){this.autoSave();}
if(this.month===18){this.showPart1Finale();return;}
if(this.month===24){this.showCheckpoint();return;}
...
let html='<div class="month-header"><h2>Month '+this.month+' — Two-Thirds There</h2></div>';
html+='<div style="text-align:center;font-size:0.8rem;color:var(--text2);line-height:1.5;max-width:420px;margin:0 auto 8px;">Part 2 is compounding. One year left — keep letting the machine run toward your final Paradise score.</div>';
html+='<div class="checkpoint-label">SCORE SO FAR</div><div class="checkpoint-score">'+composite+' <span style="font-size:0.9rem;color:var(--text2)">/ 600</span></div>';
...
html+='<button class="btn-primary" onclick="Game.continueFromCheckpoint()">▶ Continue — Finish the Run (Month 36)</button>';
html+='<div style="text-align:center;font-size:0.72rem;color:var(--text2);margin-top:6px;">One year left — let the machine compound into your final Paradise score at month 36. Ranked scores post at the <strong>Part 1 finale (month 18)</strong> and the <strong>finish (month 36)</strong>.</div>';
this.autoSave();/* checkpoint is auto-persisted */
