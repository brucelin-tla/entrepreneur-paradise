...
const PATCH_NOTES=[
{v:'0.43.1',d:'2026-06-29 20:00',n:[
'Manager model: hire a Sales Manager (Marketing) and Ops Manager (Operations) — C-suite replaced. Finance is always yours to own.',
'Team mode activates when both managers hired; they handle their categories, you focus on Finance.',
'Financial Advisor replaces CFO hire — same benefit, clearer framing.',
'Dashboard: Income/Revenue rows highlighted green, Expense red, Cash flow bold P&L summary at a glance.',
]},
...
bestAction(cat){const acts=this.getAvailableActions(cat).filter(a=>!this.isActionCompleted(a)&&!this.isActionLocked(a)&&!['hire_sales_manager','hire_ops_manager','hire_cfo','establish_board','epic_life_membership','iul_variable_loan','open_db_plan_pretax'].includes(a.id));if(!acts.length)return null;/* the variable loan & pre-tax DB plan are deliberate high-stakes player choices — never auto-picked (the safe wash/post-tax default is what the team runs) */
