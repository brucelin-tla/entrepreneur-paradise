...
renderStepIndicator(){const ac=this._activeCats||CATS,req=this._reqCats(),ri=req.indexOf(this.currentCategory),total=req.length,stepNum=ri>=0?(ri+1):total;document.getElementById('step-indicator').innerHTML='Step '+stepNum+'/'+total+': '+ac.map(c=>{const opt=!req.includes(c);if(this.selectedActions[c])return'<span style="color:var(--accent)">✓ '+CL[c]+'</span>';if(c===this.currentCategory)return'<span class="step-current">→ '+CL[c]+(opt?' (optional)':'')+'</span>';return'<span'+(opt?' style="opacity:0.55"':'')+'>'+CL[c]+(opt?' (opt)':'')+'</span>';}).join(' · ');},
...
if(!this._financeOptional()&&!this.selectedActions['finance']){if(this.currentCategory!=='finance')this.switchCategory('finance');this._focusActionList();this.showPopup('💰 Your call — the Finance move','The money decisions stay yours until you reach the <strong>Wealth stage</strong> in Finance (or switch on passive income). Pick this month\'s finance move below.');return;}
...
// Is there a finance move the player is genuinely meant to pick this month? Excludes the always-on velocity perk (members shouldn't be forced to re-engage it every month), concierge-handled actions (the Epic concierge runs those), completed and locked actions. When this is false the finance menu is effectively empty for the player.
...
// Finance is OPTIONAL (not a forced step) when a CFO/Wealth-stage can fully delegate it, OR when there's simply nothing meaningful left to pick (menu is just the velocity perk / concierge-handled / locked / completed). This stops the forced "pick a Finance move" nudge from sending the player to an empty/concierge-run menu — the exact "finance looks gated" frustration Epic members hit mid-game once the concierge has taken over the wealth machine. Early on (establish_business, build credit, etc. still pickable) finance stays required.
_financeOptional(){return this._canDelegateFinance()||!this._hasRequiredFinance();},
// The categories the player is REQUIRED to pick this month (drives step count, the Next/End button, and the "pick your move" nudges). Tabs/step labels still use _activeCats so the Finance tab stays visible as an optional extra.
_reqCats(){const ac=this._activeCats||CATS;return this._financeOptional()?ac.filter(c=>c!=='finance'):ac;},
...
if(ac.includes('finance')&&!this._financeOptional()&&!this.selectedActions['finance']){if(this.currentCategory!=='finance')this.switchCategory('finance');this._focusActionList();this.showPopup('💰 Your call — the Finance move','Your execs run marketing & operations, but the <strong>money decisions stay yours</strong> until you reach the <strong>Wealth stage</strong> in Finance (or switch on passive income). This is the part that actually builds wealth — pick this month\'s finance move below. Once you graduate, your CFO can take it over too.');return;}
...
_nextUnselectedCat(){const ac=this._reqCats(),start=Math.max(0,ac.indexOf(this.currentCategory));for(let i=0;i<ac.length;i++){const c=ac[(start+i)%ac.length];if(!this.selectedActions[c])return c;}return null;},
...
const ac=this._reqCats(),unsel=ac.filter(c=>!this.selectedActions[c]);if(!unsel.length)return this.confirmActions();const nxt=this._nextUnselectedCat();if(nxt&&nxt!==this.currentCategory){this.switchCategory(nxt);return this._focusActionList();}return this._focusActionList();/* last pick is on this screen — nudge the cards rather than end the turn */},
...
updateConfirmButton(){const btn=document.getElementById('confirm-actions-btn'),sec=document.getElementById('endturn-now-btn'),ac=this._reqCats(),total=ac.length,unsel=ac.filter(c=>!this.selectedActions[c]),done=total-unsel.length;btn.disabled=false;
...
confirmActions(){const ac=this._reqCats(),sel=ac.filter(c=>this.selectedActions[c]).length,total=ac.length;if(sel<total){const missing=ac.filter(c=>!this.selectedActions[c]).map(c=>CL[c]).join(', ');return this._confirm('End the month early?','You haven\'t chosen an action for: <strong>'+missing+'</strong> ('+sel+'/'+total+' selected).<br><br>End the month anyway?','End month',()=>this._checkBurnoutThenResolve());}
