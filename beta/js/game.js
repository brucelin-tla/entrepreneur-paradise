...
openPolicyControl(){const s=this.state,fm=v=>this.fmtMoney(v);
 ...
 const rev=Math.round(s.monthly_revenue||0),baseRate=Math.round(((s._market_rate!=null?s._market_rate:0.05)+0.01)*1000)/10,sep=this.isSeparated(),curVar=s._iul_loan_type==='variable',curActive=!!s._passive_income_active;
 ...
 h+=proj('Loan type',curVar?'Variable':'Fixed',isVar?'Variable':'Fixed');
...
{const _fr=Math.round((s._policy_fund_rate||0.15)*100),_rev=Math.round(s.monthly_revenue||0),_isVar=s._iul_loan_type==='variable';
 ...
 h+='<div class="breakdown-row"><span>Loan type</span><span>'+(_isVar?'Variable':'Fixed 3%')+'</span></div>';
...
openDealControl(cat,id){const s=this.state,fm=v=>this.fmtMoney(Math.round(v));this._dealCat=cat;
 ...
 let h='<div style="text-align:right;margin-bottom:6px;"><span onclick="Game.dealToggleInfo(\''+id+'\')" style="cursor:pointer;font-size:0.66rem;color:var(--text2);">'+infoLbl+'</span></div>';
 ...
 if(ptype==='property'){const r=this.dealRoll(id),down=Math.max(10,Math.min(50,t.downPct||20))/100;
  ...
  h+='</div>';
  h+=slider('Down payment — your cash in the deal',10,50,Math.round(down*100),Math.round(down*100)+'% = '+fm(cashDown)+' down','downPct');
  ...
  else{h+=slider('Target LTV — how much equity you pull',minL,75,Math.round(ltv*100),Math.round(ltv*100)+'% LTV = '+fm(cashOut)+' tax-free cash','ltvPct');
  h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin:8px 0 5px;">If you refinance</div><div style="padding:4px 12px;background:rgba(16,185,129,0.06);border:1px solid var(--accent);border-radius:8px;">';
 ...
 if(sel)h+='<div style="text-align:center;margin-top:6px;"><span onclick="Game.dealRemove(\''+cat+'\',\''+id+'\')" style="cursor:pointer;font-size:0.7rem;color:var(--red);">✕ Remove from this month\'s plan</span></div>';
 this.showPopup(a.label,h);},
...
{const idx=(s._index_return!=null)?s._index_return:0.0058,variable=s._iul_loan_type==='variable',washRate=0.03/12/* Fixed loan: ~3% wash */,varRate=((s._market_rate!=null?s._market_rate:0.05)+0.01)/12/* Variable loan: fed/base rate + ~1% insurer spread */;
