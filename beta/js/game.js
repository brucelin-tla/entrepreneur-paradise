...
const PATCH_NOTES=[
{v:'0.43.0',d:'2026-06-29 18:30',n:[
'C-suite replaced with a simpler Manager model: hire a Sales Manager (Marketing) and an Ops Manager (Operations). Finance is always yours to own.',
'Team mode now activates when both managers are hired — they handle their categories, you focus on Finance.',
'Financial Advisor replaces CFO hire — same benefit, clearer framing (your advisor, not an employee).',
'Dashboard rows reordered: Income/Revenue first (green), then Expense (red), then Cash flow — reads like a mini P&L at a glance.',
]},
{v:'0.42.5',d:'2026-06-29 17:41',n:[
'Dashboard redesign: Income and Revenue rows now have a green highlight, Expense rows have a red highlight — makes it instantly clear what\'s coming in vs going out. Cash flow/mo sits below as the bold net result.',
]},
...
this.month=1;this.selectedActions={};this.actionHistory=[];this.eventHistory=[];this.lifestyleHistory=[];this.monthlySnapshots=[];this._playLog=[];this._pendingTax=false;this._lost=false;this.gameOver=false;this._prevAccessible=null;this._taxRescue=0;this._ownerRescue=0;this._lastRunwayMo=null;this._atCheckpoint=false;
...
const cfRow=(label,net,id,statKey)=>{const c=net>=0?'var(--accent)':'var(--red)';const onclk=statKey?('Game.statInfo(\''+statKey+'\')'):null;const badge=statKey?(!(_vs[statKey])?'<span class="info-btn info-new">i</span>':'<span class="info-btn">i</span>'):'';return '<div'+(id?' id="'+id+'"':'')+' style="display:flex;justify-content:space-between;align-items:baseline;gap:4px;padding:3px 1px;border-bottom:1px solid rgba(127,127,127,0.14);'+(onclk?'cursor:pointer;':'')+'"'+(onclk?' onclick="'+onclk+'"':'')+'><span style="font-size:0.6rem;font-weight:700;color:var(--text2);white-space:nowrap;">'+(RICON[label]?'<span style="font-size:0.72rem;">'+RICON[label]+'</span> ':'')+label+(statKey?' '+badge:'')+'</span><span style="font-size:0.92rem;font-weight:800;color:'+c+';white-space:nowrap;">'+(net>=0?'+':'−')+fmt(Math.abs(net))+'</span></div>';};
...
P+=hlRow('Income/mo',m(persInc,persInc>0?'var(--accent)':'var(--text2)'),'rgba(16,185,129,0.10)',null,'p_income');
P+=hlRow('Expense/mo',m(persExp,'var(--red)'),'rgba(239,68,68,0.10)',null,'p_expense');
...
B+=hlRow('Revenue/mo',m(s.monthly_revenue,'var(--accent)'),'rgba(16,185,129,0.10)',null,'b_revenue');
B+=hlRow('Expense/mo',m(bizExp,'var(--red)'),'rgba(239,68,68,0.10)',null,'b_expense');
...
bestAction(cat){const acts=this.getAvailableActions(cat).filter(a=>!this.isActionCompleted(a)&&!this.isActionLocked(a)&&!['hire_sales_manager','hire_ops_manager','hire_cfo','establish_board','epic_life_membership'].includes(a.id));if(!acts.length)return null;
...
let rows='';if(s2._cro_hired)rows+=roleRow('📣','Sales Manager','marketing',(this._autoPicked||{}).marketing);if(s2._coo_hired)rows+=roleRow('⚙️','Ops Manager','operations',(this._autoPicked||{}).operations);rows+='<div style="display:flex;justify-content:space-between;gap:8px;font-size:0.76rem;padding:3px 0;border-bottom:1px solid rgba(127,127,127,0.1);"><span style="color:var(--text2);white-space:nowrap;">💰 Finance</span><span style="color:var(--gold);font-size:0.64rem;">always your pick</span></div>';
...
primaryActionBtn(){const s=this.state,team=(s._cro_hired&&s._coo_hired)||s._board_active;if(team)return this.runTeamMonth();/* managers hired: the bottom button just runs the month (managers fill any line you didn't override) */
...
{const s=this.state,team=(s._cro_hired&&s._coo_hired)||s._board_active;if(team){btn.textContent='▶ Run the Month';if(sec)sec.style.display='none';return;}}/* manager mode: one clear bottom action, no category-cycling */
