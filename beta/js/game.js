...
const ADIR={finance:[['Foundation & Credit',['establish_business','build_dnb_profile','build_personal_credit','pay_down_debt','debt_restructure','bank_personal_loan','business_credit_line','banking_relationship']],['Tax & Protection',['combined_insurance','elect_s_corp','wyoming_holding_llc','monthly_tax_reserve']],['Wealth & Passive Income',['fund_accumulation_policy','activate_passive_income','policy_loan','buy_real_estate']]],marketing:[['Your Team',['hire_sales_manager']],['Offer & Value',['build_offer','content_engine']],['Lead Generation',['cold_outreach','basic_social_content','paid_ads_test','referral_asks','influencer_megadeal']],['Sales & Conversion',['do_sales_yourself','crm_pipeline']]],operations:[['Your Team',['hire_ops_manager']],['Capacity & Delivery',['study_business_content','do_work_yourself','hire_first_contractor','fulfillment_system','rapid_offshore_scaleup']],['Systems & Freedom',['write_first_sop','project_management']],['Quality & Retention',['basic_quality_control','client_onboarding']]]};
...
(!done&&!locked&&CREDIT_APPROVAL.includes(a.id)?(()=>{const isLoan=LOAN_APPROVAL.includes(a.id),ch=this._creditApprovalChance(isLoan);if(ch>=0.6)return '';/* decent odds — no clutter, just let them apply */const msg=ch<0.35?'Approval unlikely right now':'Approval not guaranteed';return '<span class="cost-tag" style="background:rgba(239,68,68,0.12);color:var(--red);" title="'+(isLoan?'term loans are underwritten on your debt-to-income (DTI)':'revolving credit is underwritten on your personal utilization')+' + myFICO 3B — pay down balances / debt to improve your odds">⚠ '+msg+'</span>';})():'')+
