...
openPolicyControl(){const s=this.state,fm=v=>this.fmtMoney(v);
 ...
 h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:4px;">Loan type</div><div style="display:flex;gap:8px;margin-bottom:6px;">'+tBtn('wash','Fixed · 3%','borrowed slice ~washes — lapse-proof')+tBtn('variable','Index · '+baseRate+'%','flat 5% charged; full cash value keeps earning the index — higher income, lapse risk')+'</div>';
 h+='<div style="font-size:0.62rem;color:var(--text2);margin-bottom:12px;line-height:1.35;">'+(isVar?'⚠️ Index bets the index return beats the flat '+baseRate+'% loan charge — great in good years, but a downturn can compound the loan past your cash value and lapse the policy (gains become taxable).':'Fixed keeps it safe — the loan and the borrowed slice roughly wash, so the policy can\'t lapse on you.')+'</div>';
 ...
 h+=proj('Loan type',curVar?'Index':'Fixed',isVar?'Index':'Fixed');
...
{const _fr=Math.round((s._policy_fund_rate||0.15)*100),_rev=Math.round(s.monthly_revenue||0),_isVar=s._iul_loan_type==='variable';
 ...
 h+='<div class="breakdown-row"><span>Loan type</span><span>'+(_isVar?'Index 5%':'Fixed 3%')+'</span></div>';
