...
polTakeLoan(amt){const s=this.state;amt=Math.max(0,Math.min(Math.round(amt||0),this._policyBorrowable()));if(amt<=0)return;s._pending_policy_loan_amt=amt;this.hidePopup();this._paymentMethod='cash';this.selectAction('finance','policy_loan');},
polActivatePassive(){const s=this.state;if(s._passive_income_active||(s.insurance_cash_value||0)<5000)return;if(!s._iul_loan_type)s._iul_loan_type='wash';this.hidePopup();this._paymentMethod='cash';this.selectAction('finance','activate_passive_income');},
...
{const _hasPol=(s.insurance_cash_value||0)>0||(s._completed_actions||[]).includes('fund_accumulation_policy');
 ...
 h+='<div onclick="Game.openPolicyControl()" style="cursor:pointer;flex:1;border:1px solid var(--gold);background:rgba(245,200,66,0.1);border-radius:8px;padding:10px;text-align:center;"><div style="font-size:1.15rem;">🛡️</div><div style="font-size:0.72rem;font-weight:700;color:var(--gold);">Cash-Value Policy</div><div style="font-size:0.56rem;color:var(--text2);">'+(_hasPol?'Fund · loan · passive':'Open one to start')+'</div></div>';
