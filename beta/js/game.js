...
case 'buy_str':{const m=Math.max(1,Math.min(3,1+(s.monthly_revenue||0)/40000));txt='≈ '+fm(90000*m)+' property, '+fm(Math.round(90000*m*0.018))+'/mo cash flow (≈2× a rental) + a tax shield on your active income';break;}
case 'cash_out_refi':{const eq=s.real_estate_equity||0,debt=s.real_estate_debt||0,value=eq+debt,pull=Math.max(0,Math.round(value*0.75)-debt);txt='≈ '+fm(pull)+' tax-free cash pulled from your equity to redeploy (raises your LTV)';break;}
case 'private_banking':{const pledge=Math.round((s.cash||0)*0.85),draw=Math.round(pledge*0.7);txt='≈ '+fm(draw)+' borrowed against a '+fm(pledge)+' pledged portfolio — cheap dry powder, roughly carry-neutral';break;}
...
if(action.id==='buy_str'){const s=this.state,fmt=v=>this.fmtMoney(Math.round(v));const mult=Math.max(1,Math.min(3,1+(s.monthly_revenue||0)/40000));
const disc=Math.max(0.6,Math.min(1.05,s._asset_discount!=null?s._asset_discount:1));
const fairValue=Math.round((success?90000:85000)*mult),price=Math.round(fairValue*disc),mort=Math.round(price*0.8),equity=fairValue-mort;
this._addLoan('mortgage',mort);s.total_debt+=mort;s.real_estate_equity=(s.real_estate_equity||0)+equity;
...
const noi=Math.round(fairValue*(success?0.018:0.011));s.other_monthly_revenue=(s.other_monthly_revenue||0)+noi;
...
s._dyn_narrative=success?('You closed on a '+fmt(fairValue)+' short-term rental for '+fmt(price)+(disc<0.95?' — a soft market handed you a discount':'')+' and furnished it. It throws off '+fmt(noi)+'/mo — roughly double a long-term rental — and because you materially participate, the accelerated cost-seg + bonus depreciation ('+fmt(strDepr)+'/mo) can offset your ACTIVE income, a shield a normal rental can\'t give. The trade-off: it\'s a hospitality business that leans on your time, and it lives or dies by the local STR rules.'):('The deal closed on tougher terms — thinner first-season margins — but the Airbnb is yours, throwing off '+fmt(noi)+'/mo with the depreciation shield intact. Mind the local ordinances.');}
...
if(value>0){const maxDebt=Math.round(value*0.75),headroom=Math.max(0,maxDebt-debt),cashOut=Math.max(0,success?headroom:Math.round(headroom*0.55));
 ...
 }else{s._dyn_narrative='Your property is already financed near the 75% cash-out ceiling, so there\'s little equity left to pull this round. Let it appreciate or pay down other debt, then refinance later.';}
...
if(action.id==='private_banking'&&success){const s=this.state,fmt=v=>this.fmtMoney(Math.round(v)),sep=this.isSeparated(),pt=sep&&(s.personal_cash||0)>(s.cash||0)?'personal_cash':'cash';const pledge=Math.round((s[pt]||0)*0.85);if(pledge>=10000){s[pt]-=pledge;s.private_bank_balance=(s.private_bank_balance||0)+pledge;const draw=Math.round(pledge*0.7);s[pt]+=draw;s.private_bank_loan=(s.private_bank_loan||0)+draw;const moEarn=Math.round(pledge*0.004),moCost=Math.round(draw*0.0054);s._dyn_narrative='Pledged Asset Line open. You moved '+fmt(pledge)+' into a pledged portfolio that stays invested and keeps compounding (~'+fmt(moEarn)+'/mo), and drew '+fmt(draw)+' against it at about SOFR + 2% (~'+fmt(moCost)+'/mo interest). The line roughly washes on its own — the point is the '+fmt(draw)+' of cheap dry powder now in your account while your assets keep working. Deploy it into something that cash-flows. One caution a real SBLOC carries: if the market drops and your line is stretched, you can face a margin call — so don\'t lean on the whole line.';}else{s._dyn_narrative='Your Pledged Asset Line is approved, but there\'s little to pledge right now. Build up an investment or cash portfolio and the line scales with it.';}}
