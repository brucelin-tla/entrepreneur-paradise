...
openVelocityControl(){const s=this.state,fm=v=>this.fmtMoney(v);
 ...
 let h='';
 h+='<div style="text-align:right;margin-bottom:6px;"><span onclick="Game.velToggleInfo()" style="cursor:pointer;font-size:0.66rem;color:var(--text2);">ℹ️ How velocity banking works</span></div>';
 if(this._velInfo)h+='<div style="font-size:0.76rem;color:var(--text2);line-height:1.5;margin-bottom:10px;padding:8px 11px;background:var(--surface);border:1px solid var(--border);border-radius:6px;">Park your income on a line of credit and <strong>sweep every surplus dollar at your debt</strong>. Each extra dollar of principal <strong>removes the future interest</strong> that dollar would have cost over the life of the loan, so the balance clears years sooner — and on a mortgage, that paid-down principal also becomes <strong>equity</strong>.</div>';
 // Setup (one-time, uses finance move) OR the free anytime Active/Pause sweep toggle.
...
openPolicyControl(){const s=this.state,fm=v=>this.fmtMoney(v);
 ...
 /* the "Now" box merged into the single projection box below (owner: one stats box, not two) */
 if(charge>0&&borrow<=0)h+='<div style="font-size:0.7rem;color:var(--gold);margin:10px 0;line-height:1.4;">⏳ The '+fm(charge)+' surrender charge still exceeds your borrowable value — keep funding it and your borrowing opens as the cash value grows and the charge phases out.</div>';
 ...
 h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:4px;">Tax-free passive income</div>';
 if(curActive){const moP=Math.round(cv*(isVar?0.08:0.06)/12);h+='<div style="padding:8px 10px;background:rgba(16,185,129,0.1);border-left:3px solid var(--accent);border-radius:var(--radius-sm);font-size:0.74rem;color:var(--accent);font-weight:600;margin-bottom:6px;">✓ Passive income ON — ≈ '+fm(moP)+'/mo tax-free'+(isVar!==curVar?' with your staged loan type':'')+', drawn automatically each month. Index draws harder (8%) than Wash (6%).</div>';}
...
liqStage(amt){this.state._liqStaged=Math.max(0,Math.round(amt||0));if(this.state._liqStaged>0)this.state._paydownStaged=0;this.openCreditLiquidity();},
liqClear(){this.state._liqStaged=0;this.openCreditLiquidity();},
paydownStage(amt){this.state._paydownStaged=Math.max(0,Math.round(amt||0));if(this.state._paydownStaged>0)this.state._liqStaged=0;this.openCreditLiquidity();},
csConfirm(){const s=this.state;const liq=Math.max(0,Math.round(s._liqStaged||0)),pd=Math.max(0,Math.round(s._paydownStaged||0));
 if(liq>=1000){this.liquidateCredit(liq);s._liqStaged=0;this._justConfirmed='liq';}
 else if(pd>=1000){this.creditPaydown(pd);s._paydownStaged=0;this._justConfirmed='paydown';}
 else return;
 this._refreshDashboards();this.openCreditLiquidity();/* stay here — the stats box shows the result */},
openCreditLiquidity(){const s=this.state,fm=v=>this.fmtMoney(v);
 if(!s._epic_life){this._epicPanel('💵 Cash Services','<div style="font-size:0.85rem;line-height:1.6;">👑 <strong>Members-only perk.</strong> Epic Life lets your concierge liquidate your available credit into cash on demand — for a flat 6% fee, no turn used — or send cash the other way to pay down utilization.</div>');return;}
 const head=this._creditHeadroom(),pa=s.available_credit||0,ba=Math.max(0,(s.business_credit_limit||0)-(s.business_credit_used||0));
 const pdRoom=this._creditPaydownRoom();
 const staged=Math.min(Math.max(0,Math.round(s._liqStaged||0)),Math.floor(head));
 const pdStaged=Math.min(Math.max(0,Math.round(s._paydownStaged||0)),Math.floor(pdRoom));
 const row=(l,v,c)=>'<div class="breakdown-row"><span>'+l+'</span><span'+(c?' style="color:'+c+';font-weight:700;"':'')+'>'+v+'</span></div>';
 const proj=(l,b,a2,c)=>'<div class="breakdown-row"><span>'+l+'</span><span><span style="color:var(--text2)">'+b+'</span> → <strong style="color:'+(c||'var(--text)')+'">'+a2+'</strong></span></div>';
 let h='<div style="font-size:0.82rem;line-height:1.55;color:var(--text2);">Turn available credit into spendable cash for a flat <strong style="color:var(--gold);">6% fee</strong>, or send cash the other way to pay utilization down — no fee, no turn used. Pick an amount, review the projection, then <strong>Confirm</strong>.</div>';
 // ONE stats box (below Confirm) — Now + projection merged, same as the policy panel (owner)
 h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin:12px 0 5px;">Liquidate credit → cash (6% fee)</div>';
 if(head>=1000){const hmax=Math.floor(head);h+='<input type="range" min="0" max="'+hmax+'" step="'+Math.max(1,Math.round(hmax/100))+'" value="'+staged+'" style="width:100%;accent-color:var(--gold);" onchange="Game.liqStage(this.value)">';
  h+='<div style="font-size:0.72rem;font-weight:700;color:var(--gold);margin-bottom:8px;">'+fm(staged)+'</div>';}
 ...
 if(pdRoom>=1000){const pmax=Math.floor(pdRoom);h+='<input type="range" min="0" max="'+pmax+'" step="'+Math.max(1,Math.round(pmax/100))+'" value="'+pdStaged+'" style="width:100%;accent-color:var(--accent);" onchange="Game.paydownStage(this.value)">';
  h+='<div style="font-size:0.72rem;font-weight:700;color:var(--accent);margin-bottom:4px;">'+fm(pdStaged)+'</div>';}
 else h+='<div style="font-size:0.72rem;color:var(--text2);">No spare cash or revolving balance to pay down right now.</div>';
 {const _jc=this._justConfirmed==='liq'||this._justConfirmed==='paydown';const _jcLabel=this._justConfirmed==='paydown'?'✓ Confirmed — utilization paid down':'✓ Confirmed — cash landed';this._justConfirmed=null;
  if(_jc)h+='<button style="width:100%;margin-top:10px;background:var(--accent);color:#08130c;border:none;border-radius:8px;padding:11px;font-weight:800;font-size:0.82rem;">'+_jcLabel+'</button>';
  else if(staged>=1000||pdStaged>=1000)h+='<button onclick="Game.csConfirm()" style="width:100%;margin-top:10px;background:var(--gold);color:#1a1205;border:none;border-radius:8px;padding:11px;font-weight:800;font-size:0.82rem;cursor:pointer;">Confirm</button>';
  else if(head>=1000||pdRoom>=1000)h+='<div style="font-size:0.66rem;color:var(--text2);margin-top:8px;">Pick an amount above to preview the outcome — nothing happens until you Confirm.</div>';}
 h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin:12px 0 5px;">Your credit'+((staged>=1000||pdStaged>=1000)?' · if you confirm':'')+'</div><div style="padding:6px 12px;background:'+((staged>=1000||pdStaged>=1000)?'rgba(16,185,129,0.06)':'var(--surface)')+';border:1px solid '+((staged>=1000||pdStaged>=1000)?'var(--accent)':'var(--border)')+';border-radius:8px;">';
 ...
 if(staged>=1000){const fee=Math.round(staged*0.06),net=staged-fee,bd=Math.min(staged,ba),pdraw=staged-bd;
  ...
  h+=row('Cash — personal',fm(s.personal_cash||0));
 ...
 else if(pdStaged>=1000){const bizUsed=Math.max(0,s.business_credit_used||0),toBiz=Math.min(pdStaged,bizUsed),toPers=pdStaged-toBiz;
  let need=pdStaged;const fromBiz=Math.min(need,Math.max(0,s.cash||0));need-=fromBiz;const fromPers=this.isSeparated()?Math.min(need,Math.max(0,s.personal_cash||0)):0;
  h+=proj('Cash — business',fm(s.cash||0),fm((s.cash||0)-fromBiz));
  h+=proj('Cash — personal',fm(s.personal_cash||0),fm((s.personal_cash||0)-fromPers));
  h+=proj('Total debt',fm(s.total_debt||0),fm(Math.max(0,(s.total_debt||0)-pdStaged)),'var(--accent)');
  if(toBiz>0)h+=proj('Business credit used',fm(s.business_credit_used||0),fm(Math.max(0,(s.business_credit_used||0)-toBiz)),'var(--accent)');
 ...
 h+='</div>';
 if((s._credit_liquidated_total||0)>0||(s._credit_paydown_total||0)>0)h+='<div style="margin-top:10px;font-size:0.64rem;color:var(--text2);">Lifetime liquidated '+fm(s._credit_liquidated_total||0)+' · fees paid '+fm(s._credit_liquidate_fees||0)+' · paid down '+fm(s._credit_paydown_total||0)+'.</div>';
 h+='<button class="btn-secondary" style="width:100%;margin-top:8px;" onclick="Game.hidePopup()">Close</button>';
 this._epicPanel('💵 Cash Services',h);},
// ===== Owner Capital — legal, deliberate ways to move YOUR OWN money between yourself and your business (not an Epic Life perk — any LLC owner can do this in real life, member or not) =====
...
// Stage one of the three moves (mutually exclusive — staging one clears the others, same pattern as every other concierge panel).
ocStageInject(v){const s=this.state;if(!s._ocStaged)s._ocStaged={inject:0,loan:0,repay:0};s._ocStaged.inject=Math.max(0,Math.round(+v||0));if(s._ocStaged.inject>0){s._ocStaged.loan=0;s._ocStaged.repay=0;}this.openOwnerCapital();},
ocStageLoan(v){const s=this.state;if(!s._ocStaged)s._ocStaged={inject:0,loan:0,repay:0};s._ocStaged.loan=Math.max(0,Math.round(+v||0));if(s._ocStaged.loan>0){s._ocStaged.inject=0;s._ocStaged.repay=0;}this.openOwnerCapital();},
ocStageRepay(v){const s=this.state;if(!s._ocStaged)s._ocStaged={inject:0,loan:0,repay:0};s._ocStaged.repay=Math.max(0,Math.round(+v||0));if(s._ocStaged.repay>0){s._ocStaged.inject=0;s._ocStaged.loan=0;}this.openOwnerCapital();},
...
openOwnerCapital(){const s=this.state,fm=v=>this.fmtMoney(v);
 if(!this.isSeparated()){this._epicPanel('💼 Owner Capital','<div style="font-size:0.85rem;line-height:1.6;color:var(--text2);">Moving money between you and your business — legally — needs an LLC first. Complete <strong>Made It Legal</strong> (Finance → Build Your Foundation) to unlock capital injections and shareholder loans.</div>');return;}
 const pc=Math.max(0,s.personal_cash||0),bc=Math.max(0,s.cash||0),loanBal=this._ownerLoanBalance();
 if(!s._ocStaged)s._ocStaged={inject:0,loan:0,repay:0};const st=s._ocStaged;
 ...
 const row=(l,v,c)=>'<div class="breakdown-row"><span>'+l+'</span><span'+(c?' style="color:'+c+';font-weight:700;"':'')+'>'+v+'</span></div>';
 const proj=(l,b,a2,c)=>'<div class="breakdown-row"><span>'+l+'</span><span><span style="color:var(--text2)">'+b+'</span> → <strong style="color:'+(c||'var(--text)')+'">'+a2+'</strong></span></div>';
 let h='<div style="font-size:0.82rem;line-height:1.55;color:var(--text2);">Two legal ways to move your own money into the business — an <strong>equity injection</strong> (no repayment, no interest, at risk in the business) or a <strong>shareholder loan</strong> (a real ~4.5% loan the business pays back — you can call it in early too). Nothing moves until you Confirm.</div>';
 h+='<div style="font-size:0.58rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.6px;margin:12px 0 5px;">💰 Inject capital — equity, no repayment</div>';
 ...
 {const jc=this._justConfirmed,labels={oc_inject:'✓ Confirmed — capital injected',oc_loan:'✓ Confirmed — loan funded',oc_repay:'✓ Confirmed — cash returned to you'};this._justConfirmed=null;
  ...
  else if(st.inject>=1000||st.loan>=1000||st.repay>=1000)h+='<button onclick="Game.ocConfirm()" style="width:100%;margin-top:10px;background:var(--gold);color:#1a1205;border:none;border-radius:8px;padding:11px;font-weight:800;font-size:0.82rem;cursor:pointer;">Confirm</button>';
 ...
 const active=st.inject>=1000||st.loan>=1000||st.repay>=1000;
 ...
 h+='<button class="btn-secondary" style="width:100%;margin-top:8px;" onclick="Game.hidePopup()">Close</button>';
 this._epicPanel('💼 Owner Capital',h);},
...
openFinancialHealth(){const s=this.state,fm=v=>this.fmtMoney(v),sep=this.isSeparated();
 ...
 {const sr=this._survivalRunway().months;h+=row('Survival runway',!isFinite(sr)?'∞ · revenue-proof':(Math.floor(sr)+' mo'),(!isFinite(sr)||sr>=6)?'var(--accent)':(sr>=3?'var(--gold)':'var(--red)'))+'<div class="breakdown-detail">if business revenue stopped — reserves + passive vs your fixed costs</div>';}
...
const portBtn=(()=>{const doors=s._asset_units||0,pbBal=s.private_bank_balance||0,inv=s.investment_positions||0;if(doors<=0&&pbBal<=0&&inv<=0)return '';
...
s._polStaged=null;s._velStaged=null;s._fhStaged=null;s._liqStaged=0;s._paydownStaged=0;s._ocStaged=null;/* returning to the hub discards any un-confirmed staging so each panel opens fresh from current state */
...
{const _hasPol=(s.insurance_cash_value||0)>0||(s._completed_actions||[]).includes('fund_accumulation_policy');
 ...
 // Owner Capital isn't a membership perk — any LLC owner can legally inject/loan capital, so it's gated on isSeparated() (formed an LLC), not on Epic Life membership.
 h+=tile('openOwnerCapital()','💼','Owner Capital',(this._ownerLoanBalance()>0?'Inject · loan · repay':'Inject or loan capital'),'var(--accent)',this.isSeparated(),'Form an LLC first');
 h+='</div>';}
...
if(action.id==='equipment_financing'&&success){const s=this.state;
 ...
 if(basket.length){/* the whole basket in ONE finance move: downs were charged via actionCashCost → payCost; each machine finances the rest against itself. */
  ...
  for(const it of basket){
   ...
   if(it.income)s.other_monthly_revenue=(s.other_monthly_revenue||0)+it.income;
