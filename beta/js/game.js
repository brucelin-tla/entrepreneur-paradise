...
_epicRoadmapBar(locked){const D=this._epicRoadmapData();
 ...
 {const s=this.state,_pp=s._passive_income_active?this._policyPassiveMonthly():0,_pi=(s.other_monthly_revenue||0)+_pp+this._pbCarry(s),_dsvc=this.calcDebtInterest()+this.calcDebtPrincipal(),_lv=Math.round(s.living_expenses||0),_ls=Math.round(s.lifestyle_expenses||0);
  h+='<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;font-size:0.72rem;margin-top:6px;padding-top:6px;border-top:1px solid rgba(127,127,127,0.15);"><span style="color:var(--text2);">💸 Passive <strong style="color:var(--accent);">'+this.fmtMoney(Math.round(_pi))+'/mo</strong> vs '+this.fmtMoney(Math.round(_dsvc+_lv+_ls))+'/mo needed</span><span style="font-weight:800;color:'+(D.freedomPct>=100?'var(--accent)':'var(--gold)')+';">'+D.freedomPct+'% free</span></div>';}
...
openDealControl(cat,id){const s=this.state,fm=v=>this.fmtMoney(Math.round(v));this._dealCat=cat;
 ...
 else if(ptype==='pal'){const sep=this.isSeparated(),pt=sep&&(s.personal_cash||0)>(s.cash||0)?'personal_cash':'cash',pledge=Math.round((s[pt]||0)*0.85);
...
if(value>0){const maxDebt=Math.round(value*targetLTV),headroom=Math.max(0,maxDebt-debt),cashOut=Math.max(0,success?headroom:Math.round(headroom*0.55));
 if(cashOut>=1000){const pt=this.isSeparated()?'personal_cash':'cash';this._addLoan('mortgage',cashOut);s.total_debt+=cashOut;s.real_estate_equity=Math.max(0,eq-cashOut);s[pt]=(s[pt]||0)+cashOut;const newLTV=Math.round((debt+cashOut)/value*100);
...
if(action.id==='private_banking'&&success){const s=this.state,fmt=v=>this.fmtMoney(Math.round(v)),sep=this.isSeparated(),pt=sep&&(s.personal_cash||0)>(s.cash||0)?'personal_cash':'cash';const pledge=Math.round((s[pt]||0)*0.85);if(pledge>=10000){s[pt]-=pledge;s.private_bank_balance=(s.private_bank_balance||0)+pledge;
...
if(s._credit_tight&&(s.private_bank_loan||0)>0&&(s.private_bank_balance||0)>0&&!s._pal_squeeze_cycle){s._pal_squeeze_cycle=true;const _sep=this.isSeparated();
 ...
 if(ltv>0.85){const target=Math.round(marked*0.7),paydown=Math.max(0,s.private_bank_loan-target),fromPledge=Math.min(s.private_bank_balance,paydown);
  ...
  if(short>0){const pt=_sep?'personal_cash':'cash';s[pt]=(s[pt]||0)-short;s.private_bank_loan=Math.max(0,s.private_bank_loan-short);if(s[pt]<0){const unc=this.coverShortfall(-s[pt]);s[pt]=unc>0?-unc:0;}}
